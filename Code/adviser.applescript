...
on run {monitor_pid, youtubedl_pid, MacYTDL_custom_icon_file_not_posix_monitor, MacYTDL_preferences_path_monitor, YTDL_log_file_monitor, downloadsFolder_Path_monitor, diag_Title_quoted_monitor, DL_description_monitor, is_Livestream_Flag_monitor, download_filename_monitor, download_filename_new_monitor, download_filename_new_monitor_plain, YTDL_simulate_log_monitor, ADL_Clear_Batch}
	
	--	try
	
	-- NOTE: youtubedl_pid is surrogate for the pid of the running yt-dlp process.
	
	--*********************************************
	-- dialog for checking values passed from Monitor script - for some reason the next line must be blank to avoid "Scripting component error"
	...
	-- This does work
	-- set adviser_button to do shell script alerterPath & "/alerter -message " & "Just a test" & " -title " & "Some text" & " -subtitle " & "Subtitle" & " -closeLabel " & "Close" & " -timeout 10 -sender com.apple.script.id.MacYTDL -actions " & "Close 2"
	
	
	
	-- display dialog "alerterPath: " & alerterPath
	...
	on error
		try
			repeat
				-- does the yt-dlp PID exist?
				do shell script "ps -p" & youtubedl_pid
				delay 2
			end repeat
		on error
			
			-- monitor_pid and youtubedl_pid have disappeared - test for download error, advise user accordingly, show Adviser alert, open logs or downloads, or do nothing as desired		
			...
			if (YTDL_log contains "100%" and YTDL_log does not contain "ERROR:") or is_Livestream_Flag_monitor is "True" then
				...
				if adviser_button is theAdviserButtonsCloseLabel2 then
					...
					-- v1.27 - Bug somewhere in simulate step for batch downloads - exclude from playing first download until bug found
					-- v1.31, 15/4/26 - Rejigged tell block into a tell statement - removing "alias" didn't help - added delay in case Finder is slow to catch up with file system changes especially on external volumes
					if download_filename_new_monitor is not "the saved batch" then
						-- tell application "System Events" to open file pathToVideoFile     -- This gets the error can't make file into integer !
						-- tell application "System Events" to open pathToVideoFile -- This gets the error AppleEvent handler failed !
						-- tell application "System Events" to open file download_filename_play -- This gets the error Can't get the file
						delay 1
						tell application "Finder" to open pathToVideoFile
			...
			else if (YTDL_log contains "ERROR:" and YTDL_log does not contain "ffmpeg exited with code 255") or error39 is true then
				-- Download had errors	
			...
			-- If batch download initiated and user has clear batch setting turned on, parse log file for failed downloads, clear batch file then write back details for failed URLs
			if download_filename_new_monitor is "the saved batch" and ADL_Clear_Batch is "true" then
				...
				try
					-- Initialise batch_failed_downloads so required download details can be added to it
					set batch_failed_downloads to ""
					-- Set delimiter to enable downloads to be found in log file
					set AppleScript's text item delimiters to "Extracting URL: "
					set YTDL_log_batch_downloads to YTDL_log's text items
					-- Add linefeed delimiter so as to extract failed BatchFile instructions from log
					set AppleScript's text item delimiters to {"Extracting URL: ", linefeed}
					-- Check each download in log - if there was an error, add related entry in BatchFile.txt to batch_failed_downloads which is written to BatchFile.txt
					repeat with i from 1 to count YTDL_log_batch_downloads
						set batch_download to text item i of YTDL_log_batch_downloads
						if batch_download contains "ERROR:" then
							set batch_failed_URL_item to paragraph (i - 1) of batch_file_download_items
							set batch_failed_downloads to (batch_failed_downloads & batch_failed_URL_item & return)
						end if
					end repeat
					-- Write error downloads to BatchFile.txt - only if errors found otherwise clear all downloads
