...
on run {monitor_pid, youtubedl_pid, MacYTDL_custom_icon_file_not_posix_monitor, MacYTDL_preferences_path_monitor, YTDL_response_file_monitor, downloadsFolder_Path_monitor, diag_Title_quoted_monitor, DL_description_monitor, is_Livestream_Flag_monitor, download_filename_monitor, download_filename_new_monitor, download_filename_new_monitor_plain, YTDL_simulate_response_monitor}
	--on run {monitor_pid, youtubedl_pid, MacYTDL_custom_icon_file_not_posix_monitor, MacYTDL_preferences_path_monitor, YTDL_response_file_monitor, downloadsFolder_Path_monitor, diag_Title_quoted_monitor, DL_description_monitor, is_Livestream_Flag_monitor, download_filename_monitor, download_filename_new_monitor, download_filename_new_monitor_plain, YTDL_simulate_response_monitor}
	...
	--display dialog "monitor_pid: " & monitor_pid & return & "MacYTDL_custom_icon_file_not_posix_monitor: " & MacYTDL_custom_icon_file_not_posix_monitor & return & "MacYTDL_preferences_path_monitor: " & MacYTDL_preferences_path_monitor & return & "YTDL_response_file_monitor: " & YTDL_response_file_monitor & return & "downloadsFolder_Path_monitor: " & downloadsFolder_Path_monitor & return & "diag_Title_quoted_monitor: " & diag_Title_quoted_monitor & return & "DL_description_monitor: " & DL_description_monitor & return & "is_Livestream_Flag_monitor" & is_Livestream_Flag_monitor & return & "download_filename_monitor: \"" & download_filename_monitor & "\"" & return & "download_filename_new_monitor: " & download_filename_new_monitor & return & "download_filename_new_monitor_plain: " & download_filename_new_monitor_plain & return & "YTDL_simulate_response_monitor: \"" & YTDL_simulate_response_monitor & "\"" with title "Adviser"
	--*********************************************	
	...
	on error
		...
		on error
			...
			on error errmsg
				display dialog "Error in reading response file: " & YTDL_response_file_monitor & return & "The error reported was " & errmsg
			end try
			if (YTDL_response contains "100%" or is_Livestream_Flag_monitor is "True") and YTDL_response does not contain "ERROR:" then
				...
				if (YTDL_response contains "Requested formats are incompatible for merge and will be merged into mkv" or YTDL_response contains "doesn't support embedding a thumbnail, mkv will be used") and YTDL_response does not contain "Converting video from mkv to " then
				...
				if adviser_button is theAdviserButtonsCloseLabel2 then
					if download_filename_play is "No File Name" then
						set theAdviserNoFileNameLabel to localized string "Sorry, can't play files from a batch download." in bundle file pathToBundleShort from table "MacYTDL"
						display dialog theAdviserNoFileNameLabel buttons {theAdviserButtonsOKLabel} default button 1 with title diag_Title_quoted_monitor with icon file MacYTDL_custom_icon_file_not_posix_monitor giving up after 7
					else
						-- Did user specify download location in custom template - if so, downloadsFolder_Path_monitor can be omitted
						-- If download is a recorded live stream, get file name from response file as it can be different to name found in simulation - beware of date-time stamps
						if is_Livestream_Flag_monitor is "True" then
							repeat with find_paragraph in paragraphs of YTDL_response
						...
						if download_filename_play contains "Users" then
							...
							-- set pathToVideoFile to download_filename_play
						else
							set pathToVideoFile to POSIX file (downloadsFolder_Path_monitor & "/" & download_filename_play)
							-- set pathToVideoFile to (downloadsFolder_Path_monitor & "/" & download_filename_play)
						end if
						tell application "Finder"
							-- tell application "System Events" to open file pathToVideoFile
				...
				if adviser_button is text 2 thru -2 of theAdviserButtonsCloseLabel3 then
					...
					set theFile to POSIX path of YTDL_response_file_monitor
					openFile(theFile)
					--tell application "Finder"
			...
			else if YTDL_response does not contain "ERROR:" and YTDL_response does not contain "100%" and YTDL_response does not contain "has already been downloaded" and is_Livestream_Flag_monitor is "False" then
