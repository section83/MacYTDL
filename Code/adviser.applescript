...
on run {monitor_pid, youtubedl_pid, MacYTDL_custom_icon_file_not_posix_monitor, MacYTDL_preferences_path_monitor, YTDL_log_file_monitor, downloadsFolder_Path_monitor, diag_Title_quoted_monitor, DL_description_monitor, is_Livestream_Flag_monitor, download_filename_monitor, download_filename_new_monitor, download_filename_new_monitor_plain, YTDL_simulate_log_monitor}
	...
	--display dialog "monitor_pid: " & monitor_pid & return & "MacYTDL_custom_icon_file_not_posix_monitor: " & MacYTDL_custom_icon_file_not_posix_monitor & return & "MacYTDL_preferences_path_monitor: " & MacYTDL_preferences_path_monitor & return & "YTDL_log_file_monitor: " & YTDL_log_file_monitor & return & "downloadsFolder_Path_monitor: " & downloadsFolder_Path_monitor & return & "diag_Title_quoted_monitor: " & diag_Title_quoted_monitor & return & "DL_description_monitor: " & DL_description_monitor & return & "is_Livestream_Flag_monitor" & is_Livestream_Flag_monitor & return & "download_filename_monitor: \"" & download_filename_monitor & "\"" & return & "download_filename_new_monitor: " & download_filename_new_monitor & return & "download_filename_new_monitor_plain: " & download_filename_new_monitor_plain & return & "YTDL_simulate_log_monitor: \"" & YTDL_simulate_log_monitor & "\"" with title "Adviser"
	--*********************************************	
	...
	on error
		...
		on error
			...
			on error errmsg
				display dialog "Error in reading log file: " & YTDL_log_file_monitor & return & "The error reported was " & errmsg
			end try
			if (YTDL_log contains "100%" or is_Livestream_Flag_monitor is "True") and YTDL_log does not contain "ERROR:" then
				...
				if adviser_button is theAdviserButtonsCloseLabel2 then
					if download_filename_play is "No File Name" then
						...
						if is_Livestream_Flag_monitor is "True" then
							...
							display dialog "download_filename_play: " & download_filename_play
			...
			else if YTDL_log contains "ERROR:" then
				-- Download had errors	
				...
				if adviser_button is text 2 thru -2 of theAdviserButtonsCloseLabel3 then
					...
					openFile(theFile)
					--tell application "Finder"
					--	open YTDL_log_file_monitor as alias
					--end tell
