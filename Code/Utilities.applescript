...
on install_Myriad(Myriad_file, path_to_MacYTDL, resourcesPath)
	...
	do shell script resourcesPath & "alerter -message " & installAlertMessage & " -title " & installAlertTitle & " -subtitle " & installAlertSubtitle & " -timeout 3 -sender com.apple.script.id.MacYTDL -actions " & installAlertActionLabel & " > /dev/null 2> /dev/null & "
	set libraries_folder to (POSIX path of (path to home folder) & "Library/Script Libraries/")
	set libraries_folder_quoted to quoted form of libraries_folder
...
on set_admin_settings(old_version_prefs, diag_Title, theButtonNoLabel, theButtonYesLabel, MacYTDL_prefs_file, MacYTDL_version, MacYTDL_date, MacYTDL_preferences_path, path_to_MacYTDL, X_position, Y_position, theBestLabel, theDefaultLabel, theNoRemuxLabel, resourcesPath, show_yt_dlp, YTDL_version, MacYTDL_custom_icon_file, theButtonCancelLabel, window_Position, theButtonReturnLabel, MacYTDL_custom_icon_file_posix)
	...
	set {theButtons, minWidth} to create buttons {theButtonCancelLabel, theSettingsButtonLabel, theButtonsSaveLabel} button keys {".", "s", ""} default button 3
	...
	set theCheckboxNoWarningsLabel to localized string "Suppress warnings from YT-DLP" from table "MacYTDL"
	...
	if settings_button_number_returned is 3 or settings_button_number_returned is 2 then -- Save Settings
		...
		set settings_QT_Compat_choice to item 6 of settings_controls_results -- <= Make download QT compatible
		set settings_parallel_choice to item 7 of settings_controls_results -- <= Download multiple videos in parallel
		set settings_limit_rate_choice to item 8 of settings_controls_results -- <= Limit rate choice
		set settings_limit_rate_value_choice to item 9 of settings_controls_results -- <= Limit rate value choice
		set settings_auto_download_choice to item 10 of settings_controls_results -- <= Automatic download using Service
		set settings_show_settings_choice to item 11 of settings_controls_results -- <= Show settings before download choice
		set settings_YTDL_auto_choice to item 12 of settings_controls_results -- <= Auto check YTDL version on startup choice
		set settings_timestamps_choice to item 13 of settings_controls_results -- <= Add timestamps choice
		set settings_verbose_choice to item 14 of settings_controls_results -- <= Verbose choice
