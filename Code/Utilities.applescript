...
-- If version of DTP library is old, replace with new - commented out old code to delete DTP library used before the name change to test if anyone complains
	...
	if show_yt_dlp is "yt-dlp" then
		try
			...
			if YTDL_releases_page does not contain "yt-dlp_macos" then
				set theYTDLCantInstallLabel to localized string "The latest yt-dlp Python script is not available. Will download the 8/3/22 version." in bundle file path_to_MacYTDL from table "MacYTDL"
				...
				set curl_YTDLP to ("curl -L https://github.com/yt-dlp/yt-dlp/releases/download/2022.03.08.1/yt-dlp_macos" & " -o /usr/local/bin/yt-dlp")
			else
...
on check_ffmpeg_installed(theButtonOKLabel, diag_Title, path_to_MacYTDL, usr_bin_folder, ffmpeg_exists, ffprobe_exists, resourcesPath, MacYTDL_custom_icon_file, user_on_old_os)
...
on check_settings(MacYTDL_prefs_file, old_version_prefs, diag_Title, theButtonNoLabel, theButtonYesLabel, MacYTDL_version, MacYTDL_date, MacYTDL_preferences_path, path_to_MacYTDL, X_position, Y_position, theBestLabel, theDefaultLabel, resourcesPath, show_yt_dlp, YTDL_version)
	tell application "System Events"
		try
			tell property list file MacYTDL_prefs_file
			...
			if test_DL_subtitles is "Yes" or test_DL_subtitles is "No" then
				...
				my set_preferences(old_version_prefs, diag_Title, theButtonNoLabel, theButtonYesLabel, MacYTDL_prefs_file, MacYTDL_version, MacYTDL_date, MacYTDL_preferences_path, path_to_MacYTDL, X_position, Y_position, theBestLabel, theDefaultLabel, resourcesPath, show_yt_dlp, YTDL_version)
			end if
		on error
		...
		tell property list file MacYTDL_prefs_file
			if not (exists property list item "Subtitles_YTAuto") then
				my add_v1_16_preference(MacYTDL_prefs_file, theDefaultLabel)
			end if
		end tell
...
on check_for_v1_21_settings(MacYTDL_prefs_file, DL_Use_YTDLP, MacYTDL_preferences_path, youtubedl_file, ytdlp_file)
	tell application "System Events"
		tell property list file MacYTDL_prefs_file
...
end check_for_v1_21_settings

---------------------------------------------------
--
-- 		Check current settings are up-to-date
--
---------------------------------------------------
-- Handler for checking preferences file is up-to-date - called by main_dialog if it gets an error reading prefs file
on check_for_old_prefs(MacYTDL_prefs_file, show_yt_dlp, YTDL_version, MacYTDL_preferences_path, DL_Use_YTDLP, youtubedl_file, ytdlp_file)
	tell application "System Events"
		-- Check on need to add new v1.17 proxy settings to the prefs file
		tell property list file MacYTDL_prefs_file
			if not (exists property list item "Use_Proxy") then
	...
	-- Check on need to add new v1.21 YTDL/YT-DLP version + save/restore settings to the prefs file
	tell application "System Events"
		tell property list file MacYTDL_prefs_file
			if not (exists property list item "YTDL_YTDLP_version") then
				if DL_Use_YTDLP is "youtube-dl" then
...
on set_preferences(old_version_prefs, diag_Title, theButtonNoLabel, theButtonYesLabel, MacYTDL_prefs_file, MacYTDL_version, MacYTDL_date, MacYTDL_preferences_path, path_to_MacYTDL, X_position, Y_position, theBestLabel, theDefaultLabel, resourcesPath, show_yt_dlp, YTDL_version, MacYTDL_custom_icon_file)
	set installAlertActionLabel to quoted form of "_"
	set installAlertTitle to quoted form of (localized string "MacYTDL installation:" from table "MacYTDL")
...
on Get_SBS_Episodes(URL_user_entered, diag_Title, theButtonOKLabel, theButtonCancelLabel, theButtonDownloadLabel, X_position, screen_height, MacYTDL_custom_icon_file, MacYTDL_custom_icon_file_posix, theButtonReturnLabel, skip_Main_dialog, path_to_MacYTDL)
	...
	-- Get season number
	-- In v1.19.1, changed location searched for season number
	-- set start_SBS_seasonnumber to (offset of "class=\"MuiTab-wrapper\">Season " in SBS_show_page) + 30
	-- set end_SBS_seasonnumber to (offset of "</span><span class=\"jss" in SBS_show_page) - 1
	...
	-- Populate the lists of names and URLs - Repeat for each occurrence of an episode found in the show page - first text item is all text before first occurrence - need to find and replace special character codes for ampersand and comma
	repeat with i from 1 to (myNum - 1)
		...
		if mediaTitle begins with SBS_show_name then
			...
			-- Problem: some season numbers are more than 1 character long - some are "2021" for e.g.
			-- Decided to assume that the word Episode appears in every mediaTitle item
