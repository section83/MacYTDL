...
-- v1.30 - First time users now get stable yt-dlp release
-- v1.31 - Users on 10.14 - are pointed to correct Legacy location - last release is 2025.08.11
	...
	try
		...
		try
			if show_yt_dlp is "yt-dlp" then
				...
				-- v1.31 - 1/4/26 - Move following block into this if/end loop as it's not needed for legacy releases
				...
				set AppleScript's text item delimiters to ""
				-- v1.30, 25/12/25 - Added the return statement as it is required to get version number into plist
				return YTDL_version
			end if
			
			-- User on 10.9-10.14.6 - install yt-dlp-legacy
			-- v1.29 - Now getting latest yt-dlp instead of checking for it
			-- v1.31, 12/2/26 - Change to correct URL for the final Legacy build
			-- v1.31 - 1/4/26 - Hard code the yt-dlp verison number for legacy build
			if show_yt_dlp is "yt-dlp-legacy" then
				...
				set YTDL_version to "2025.08.11"
				return YTDL_version
			end if
		on error number 6
	...
	if FFmpeg_page is "" then
		...
		if release_build_version_FFmpeg is not equal to ffmpeg_version or release_build_version_FFmpeg is not equal to ffprobe_version then
			...
			if ffmpeg_install_answ is theButtonYesLabel then
				...
				set alert_text_ffmpeg to theFFmpegProbeAlertOutOfDateLabel & ffmpeg_version
			end if
		end if
	end if
	return alert_text_ffmpeg
...
-- Most recently added setting used for testing is Get_Cookies
-- v1.31 - Added get_gookies, cookies_source, netrc_location, use_netrc_location, cookies_from_browser, set_merge_format, merge_format
on check_settings(diag_Title, theButtonNoLabel, theButtonYesLabel, MacYTDL_version, MacYTDL_date, MacYTDL_preferences_path, path_to_MacYTDL, resourcesPath, show_yt_dlp, MacYTDL_prefs_file, theBestLabel, theDefaultLabel, X_position, Y_position, theNoRemuxLabel, MacYTDL_custom_icon_file)
	tell application "System Events"
		try
			tell property list file MacYTDL_prefs_file
				if not (exists property list item "Get_Cookies") then
...
-- v1.31, 15/2/25 - Added new settings: Get_Cookies (toggle) and Cookies_Source (browser)
-- v1.31, 11/4/25 - Added new settings for the playlist items limit but retained get_cookies test
-- v1.31, 21/4/26 - Added new setting for netrc location
-- v1.31, 1/5/26 - Added new settings for setting and storing merged output format
on update_settings(MacYTDL_prefs_file, theBestLabel, theDefaultLabel, X_position, Y_position, theNoRemuxLabel, show_yt_dlp, MacYTDL_preferences_path)
	tell application "System Events"
		tell property list file MacYTDL_prefs_file
			...
			if exists property list item "Cookies_Source" then
				set DL_Cookies_Source to value of property list item "Cookies_Source"
			else
				set DL_Cookies_Source to ""
			end if
			...
			if exists property list item "Get_Cookies" then
				set DL_Get_Cookies to value of property list item "Get_Cookies"
			else
				set DL_Get_Cookies to false
			end if
			...
			if exists property list item "Merge_Format" then
				set DL_Merge_Format to value of property list item "Merge_Format"
			else
				set DL_Merge_Format to false
			end if
			...
			if exists property list item "Netrc_Location" then
				set DL_Netrc_Location to value of property list item "Netrc_Location"
			else
				set DL_Netrc_Location to ""
			end if
			...
			if exists property list item "PL_Items_Limit" then
				set DL_PL_Items_Limit to value of property list item "PL_Items_Limit"
			else
				set DL_PL_Items_Limit to 20
			end if
			...
			if exists property list item "Set_Merge_Format" then
				set DL_Set_Merge_Format to value of property list item "Set_Merge_Format"
			else
				set DL_Set_Merge_Format to false
			end if
			...
			if exists property list item "Use_netrc_location" then
				set DL_Use_Netrc_Location to value of property list item "Use_netrc_location"
			else
				set DL_Use_Netrc_Location to false
			end if
	...
	tell application "System Events"
		...
		tell property list items of thePropertyListFile
			...
			make new property list item at end with properties {kind:string, name:"Cookies_Source", value:DL_Cookies_Source}
			...
			make new property list item at end with properties {kind:boolean, name:"Get_Cookies", value:DL_Get_Cookies}
			...
			make new property list item at end with properties {kind:string, name:"Merge_Format", value:theDefaultLabel}
			make new property list item at end with properties {kind:string, name:"Name_Of_Settings_In_Use", value:DL_Settings_In_Use}
			make new property list item at end with properties {kind:string, name:"Netrc_Location", value:DL_Netrc_Location}
			...
			make new property list item at end with properties {kind:integer, name:"PL_Items_Limit", value:DL_PL_Items_Limit}
			...
			make new property list item at end with properties {kind:boolean, name:"Set_Merge_Format", value:false}
			...
			make new property list item at end with properties {kind:boolean, name:"Use_netrc_location", value:DL_Use_Netrc_Location}
...
-- v1.31, 15/2/26 - Added Get_Cookies (toggle) and Cookies_Source (browser)
-- v1.31 - 11/4/26 - Added playlist item limit
on set_preferences(old_version_prefs, diag_Title, theButtonNoLabel, theButtonYesLabel, MacYTDL_prefs_file, MacYTDL_version, MacYTDL_date, MacYTDL_preferences_path, path_to_MacYTDL, X_position, Y_position, theBestLabel, theDefaultLabel, theNoRemuxLabel, resourcesPath, show_yt_dlp, MacYTDL_custom_icon_file)
	...
	set theNoCookielFileLabel to localized string "No Cookie File" from table "MacYTDL"
	set theNoChangeLabel to localized string "No change" from table "MacYTDL"
	tell application "System Events"
		set thePropertyListFile to make new property list file with properties {name:MacYTDL_prefs_file}
		tell property list items of thePropertyListFile
			...
			make new property list item at end with properties {kind:string, name:"Cookies_Location", value:("/" & theNoCookielFileLabel)}
			...
			make new property list item at end with properties {kind:string, name:"Deno_version", value:deno_version}
			make new property list item at end with properties {kind:boolean, name:"Get_Cookies", value:false}
			make new property list item at end with properties {kind:string, name:"Cookies_Source", value:"Safari"}
			make new property list item at end with properties {kind:integer, name:"PL_Items_Limit", value:20}
			make new property list item at end with properties {kind:boolean, name:"Use_netrc_location", value:false}
			make new property list item at end with properties {kind:string, name:"Netrc_Location", value:""}
			make new property list item at end with properties {kind:boolean, name:"Set_Merge_Format", value:false}
			make new property list item at end with properties {kind:string, name:"Merge_Format", value:theDefaultLabel}
		end tell
	end tell
...
on set_admin_settings(old_version_prefs, diag_Title, theButtonNoLabel, theButtonYesLabel, MacYTDL_prefs_file, MacYTDL_version, MacYTDL_date, MacYTDL_preferences_path, path_to_MacYTDL, X_position, Y_position, theBestLabel, theDefaultLabel, theNoRemuxLabel, resourcesPath, show_yt_dlp, YTDL_version, MacYTDL_custom_icon_file, theButtonCancelLabel, window_Position, theButtonReturnLabel, MacYTDL_custom_icon_file_posix, theButtonOKLabel)
	...
	set {settings_theCheckbox_No_Warnings, theTop} to create checkbox theCheckboxNoWarningsLabel & " " & "yt-dlp" left inset 70 bottom (theTop + 10) max width 200 initial state DL_No_Warnings
	...
	set theFieldItemsLimitLabel to localized string "Playlist items soft limit"
	set {settings_theLabel_Field_Items_Limit, theTop} to create label theFieldItemsLimitLabel left inset 105 bottom (theTop + 3) max width 150
	set {settings_theField_Items_Limit, theTop} to create field DL_PL_Items_Limit left inset 65 bottom (theTop - 20) field width 37
	
	set theFieldnetrcLocationLabel to localized string "Custom netrc location" from table "MacYTDL"
	set {settings_theField_netrc_location, theTop} to create field DL_Netrc_Location left inset 210 bottom (theTop + 3) field width 225 placeholder text theFieldnetrcLocationLabel
	set theCheckboxUsenetrclocationLabel to localized string "Custom location" from table "MacYTDL"
	set {settings_theCheckbox_Use_netrc_Location, theTop} to create checkbox theCheckboxUsenetrclocationLabel left inset 250 bottom (theTop + 2) max width 100 initial state DL_Use_Netrc_Location
	set theCheckboxUsenetrcLabel to localized string "Use .netrc credentials" from table "MacYTDL"
	set {settings_theCheckbox_Use_netrc, theTop} to create checkbox theCheckboxUsenetrcLabel left inset 70 bottom (theTop - 18) max width 200 initial state DL_Use_netrc
	...
	set settings_allControls to {theSettingsRule, settings_theCheckbox_No_Warnings, settings_theCheckbox_Use_CustomSettings, settings_theField_Custom_Settings, settings_theLabel_Field_Items_Limit, settings_theField_Items_Limit, settings_theCheckbox_Use_netrc_Location, settings_theField_netrc_location, settings_theCheckbox_Use_netrc, settings_theCheckbox_Delete_Partial, settings_theCheckbox_Use_Parts, settings_theCheckbox_Discard_URL, settings_theCheckbox_ClearBatch, settings_theCheckbox_QT_Compat, settings_theCheckbox_Parallel, settings_theCheckbox_Limit_Rate, settings_theField_LimitRateValue, settings_theCheckbox_Auto_Download, settings_theCheckbox_Show_Settings, settings_theCheckbox_Auto_YTDL_Check, settings_theCheckbox_TimeStamps, settings_theCheckbox_Verbose, MacYTDL_icon, settings_prompt}
	
	-- allControls before adding custom settings controls
	--	set settings_allControls to {theSettingsRule, settings_theCheckbox_No_Warnings, settings_theLabel_Field_Items_Limit, settings_theField_Items_Limit, settings_theCheckbox_Use_netrc_Location, settings_theField_netrc_location, settings_theCheckbox_Use_netrc, settings_theCheckbox_Delete_Partial, settings_theCheckbox_Use_Parts, settings_theCheckbox_Discard_URL, settings_theCheckbox_ClearBatch, settings_theCheckbox_QT_Compat, settings_theCheckbox_Parallel, settings_theCheckbox_Limit_Rate, settings_theField_LimitRateValue, settings_theCheckbox_Auto_Download, settings_theCheckbox_Show_Settings, settings_theCheckbox_Auto_YTDL_Check, settings_theCheckbox_TimeStamps, settings_theCheckbox_Verbose, MacYTDL_icon, settings_prompt}
	...
	if settings_button_number_returned is 3 or settings_button_number_returned is 2 then -- Save Admin Settings - optionally go to Settings dialog
		...
		set settings_use_custom_settings_choice to item 3 of settings_controls_results -- <= Use custom settings choice
		set settings_custom_settings_choice to item 4 of settings_controls_results -- <= The custom settings to use
		-- set settings_choice_5 to item 5 of settings_controls_results -- <= Playlist items soft limit label
		set settings_PL_items_limit_choice to item 6 of settings_controls_results -- <= Playlist items soft limit
		set settings_use_netrc_location_choice to item 7 of settings_controls_results -- <= Use Custom location for .netrc file
		set settings_netrc_location_choice to item 8 of settings_controls_results -- <= Custom location for .netrc file
		set settings_netrc_choice to item 9 of settings_controls_results -- <= Use credentials in the .netrc file
		set settings_Delete_Partial_choice to item 10 of settings_controls_results -- <= Delete partial downloads choice
		set settings_DontUseParts_choice to item 11 of settings_controls_results -- <= Use parts in DL file names choice
		set settings_Discard_URL_choice to item 12 of settings_controls_results -- <= Discard URL on download choice
		set settings_Clear_Batch_choice to item 13 of settings_controls_results -- <= Discard URL on download choice
		set settings_QT_Compat_choice to item 14 of settings_controls_results -- <= Make download QT compatible
		set settings_parallel_choice to item 15 of settings_controls_results -- <= Download multiple videos in parallel
		set settings_limit_rate_choice to item 16 of settings_controls_results -- <= Limit rate choice
		set settings_limit_rate_value_choice to item 17 of settings_controls_results -- <= Limit rate value choice
		set settings_auto_download_choice to item 18 of settings_controls_results -- <= Automatic download using Service
		set settings_show_settings_choice to item 19 of settings_controls_results -- <= Show settings before download choice
		set settings_YTDL_auto_choice to item 20 of settings_controls_results -- <= Auto check YTDL version on startup choice
		set settings_timestamps_choice to item 21 of settings_controls_results -- <= Add timestamps choice
		set settings_verbose_choice to item 22 of settings_controls_results -- <= Verbose choice
		...
		tell application "System Events"
			tell property list file MacYTDL_prefs_file
				...
				set value of property list item "Netrc_Location" to settings_netrc_location_choice
				set value of property list item "Limit_Rate_Value" to settings_limit_rate_value_choice
			end tell
		end tell
		
		-- Better for Limit rate to be always 0.0 or value set by user 
		if settings_limit_rate_value_choice is less than 0.0 then
			tell application "System Events"
				tell property list file MacYTDL_prefs_file
		...
		-- v1.31, 2/5/26 - Moved custom settings from Settings dialog
		...
		-- v1.31, 23/4/26 - Error check the netrc settings selected - settings_use_netrc_location_choice and settings_netrc_location_choice are declared global
		-- If user wants to use custom netrc location, make sure they have supplied location - well, no real error checking on nonsense text but better than nothing
		-- Also, make sure user has turned on the .netrc setting as custom location only used if .netrc credentials are used
		if settings_netrc_choice is false and settings_use_netrc_location_choice is true then
			set theNoNetrcLabel to localized string "Sorry, you can only turn on custom .netrc location if using .netrc credentials." from table "MacYTDL"
			set theDecideNetrcLabel to localized string "Would you like .netrc turned on or location turned off ?" from table "MacYTDL"
			set theButtonNetrcOnLabel to localized string ".netrc On" from table "MacYTDL"
			set theButtonnLocOffLabel to localized string "location Off" from table "MacYTDL"
			set theDecideNetrcLabel to button returned of (display dialog theNoNetrcLabel & return & return & theDecideNetrcLabel with title diag_Title buttons {theButtonNetrcOnLabel, theButtonnLocOffLabel} default button 1 with icon file MacYTDL_custom_icon_file giving up after 600)
			--	set_admin_settings(old_version_prefs, diag_Title, theButtonNoLabel, theButtonYesLabel, MacYTDL_prefs_file, MacYTDL_version, MacYTDL_date, MacYTDL_preferences_path, path_to_MacYTDL, X_position, Y_position, theBestLabel, theDefaultLabel, theNoRemuxLabel, resourcesPath, show_yt_dlp, YTDL_version, MacYTDL_custom_icon_file, theButtonCancelLabel, window_Position, theButtonReturnLabel, MacYTDL_custom_icon_file_posix, theButtonOKLabel)
			if theDecideNetrcLabel is theButtonNetrcOnLabel then
				set settings_netrc_choice to true
				tell application "System Events"
					tell property list file MacYTDL_prefs_file
						set value of property list item "Use_netrc" to true
					end tell
				end tell
			end if
			if theDecideNetrcLabel is theButtonnLocOffLabel then set settings_use_netrc_location_choice to false
		end if
		if settings_netrc_choice is true and settings_use_netrc_location_choice is true then
			if settings_netrc_location_choice is "" then
				set theNoNetrcLocationProvidedLabel to localized string "Sorry, you need to supply a location if you wish to use a custom .netrc location." from table "MacYTDL"
				display dialog theNoNetrcLocationProvidedLabel with title diag_Title buttons {theButtonOKLabel} default button 1 with icon file MacYTDL_custom_icon_file giving up after 600
				set settings_use_netrc_location_choice to false
				set_admin_settings(old_version_prefs, diag_Title, theButtonNoLabel, theButtonYesLabel, MacYTDL_prefs_file, MacYTDL_version, MacYTDL_date, MacYTDL_preferences_path, path_to_MacYTDL, X_position, Y_position, theBestLabel, theDefaultLabel, theNoRemuxLabel, resourcesPath, show_yt_dlp, YTDL_version, MacYTDL_custom_icon_file, theButtonCancelLabel, window_Position, theButtonReturnLabel, MacYTDL_custom_icon_file_posix, theButtonOKLabel)
			end if
			-- Now can go ahead and save netrc location settings - must test value of settings_use_netrc_location_choice to avoid unwinding recursion causing faulty values getting into plist
			if settings_use_netrc_location_choice is true then
				tell application "System Events"
					tell property list file MacYTDL_prefs_file
						set value of property list item "Use_netrc_location" to true
						set value of property list item "Netrc_Location" to settings_netrc_location_choice
					end tell
				end tell
			end if
			...
			tell application "System Events"
				tell property list file MacYTDL_prefs_file
					set value of property list item "Use_netrc_location" to false
					--	set value of property list item "Netrc_Location" to ""
				end tell
			end tell
		end if
		
		-- Trying to enable user to save the location even tho setting is off	
		-- display dialog "settings_netrc_location_choice: " & settings_netrc_location_choice
		--	if settings_netrc_location_choice is "" then
		--		tell application "System Events"
		--			tell property list file MacYTDL_prefs_file
		--				set value of property list item "Netrc_Location" to ""
		...
		-- If user blanks PL_Items_Limit or sets to zero, reset to default of 20 - otherwise update setting with number in the field
		-- User can avoid limit by entering an extremely large number e.g. 500
		-- Must also be an integer
		set settings_PL_items_limit_choice to settings_PL_items_limit_choice as integer
		if settings_PL_items_limit_choice is 0 or settings_PL_items_limit_choice is "" then
			set DL_PL_Items_Limit to 20 as integer
		else
			set DL_PL_Items_Limit to settings_PL_items_limit_choice as integer
		end if
		tell application "System Events"
			tell property list file MacYTDL_prefs_file
				set value of property list item "PL_Items_Limit" to DL_PL_Items_Limit
			end tell
		end tell
...
-- v1.31, 7/6/26 - Changed remux_format_choice to DL_Remux_format - don't know why this was not a bug before
-- v1.31, 7/6/26 - Added --net-location and Deno
on show_settings(YTDL_subtitles, DL_Remux_original, DL_YTDL_auto_check, DL_STEmbed, DL_audio_only, YTDL_description, DL_Limit_Rate, DL_over_writes, DL_Thumbnail_Write, DL_verbose, DL_Thumbnail_Embed, DL_Add_Metadata, DL_Use_Proxy, DL_Use_Cookies, DL_Use_Custom_Template, DL_Use_Custom_Settings, DL_Remux_format, DL_TimeStamps, DL_Use_YTDLP, DL_Parallel, DL_discard_URL, DL_Dont_Use_Parts, DL_No_Warnings, YTDL_version, folder_chosen, theButtonQuitLabel, theButtonCancelLabel, theButtonDownloadLabel, DL_Show_Settings, MacYTDL_prefs_file, MacYTDL_custom_icon_file_posix, diag_Title, YTDL_Use_netrc, DL_Remux_Recode, DL_Netrc_Location, deno_version)
	...
	if DL_Netrc_Location is "" then
		set MDDL_netrc_location to "Not set"
	else
		set MDDL_netrc_location to DL_Netrc_Location
	end if
	...
	set theShowSettingsPromptTextDenoLabel to localized string "Deno version:" from table "MacYTDL"
...
-- v1.31, 31/5/26 - Added special character 8239 which is placed before the am/pm text by the current date call
on get_Date_Time()
	set download_date_time to (current date) as string
	set am_pm_separator to (character id 8239)
	set AppleScript's text item delimiters to {", ", " ", ":", am_pm_separator}
	--	set AppleScript's text item delimiters to {", ", " ", ":", ","}
...
on show_about(DL_Use_YTDLP, MacYTDL_date, theButtonOKLabel, diag_Title, MacYTDL_custom_icon_file_posix)
	...
	set theButtonsAbout4Label to localized string "With thanks to Shane Stanley, Mark Alldritt, fellow AppleScripters and all MacYTDL users." from table "MacYTDL"
