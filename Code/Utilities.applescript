...
on auto_Download(MacYTDL_prefs_file, URL_user_entered_clean, path_to_MacYTDL)
	...
	if DL_Use_YTDLP is "yt-dlp-legacy" then
		set general_DL_Use_YTDLP to "yt-dlp"
	else
		set general_DL_Use_YTDLP to DL_Use_YTDLP
	end if
...
---------------------------------------------------------------------------------------------------------------------------------------------
--
-- 	Handlers to update format of Preferences file for v1.10, 1.11, 1.12.1, 1.16, 1.17, 1.18, 1.19, 1.20, 1.21, 1.23 and v1.24
--
---------------------------------------------------------------------------------------------------------------------------------------------
...
-- Handler to show format chooser - user can choose which available formats are to be downloaded
on get_formats_list(URL_user_entered, diag_Title, theButtonCancelLabel, theButtonDownloadLabel, X_position, screen_height, MacYTDL_custom_icon_file, MacYTDL_custom_icon_file_posix, theButtonReturnLabel, skip_Main_dialog, path_to_MacYTDL, DL_Use_YTDLP, shellPath, download_filename_formats, YTDL_credentials)
	
	set formats_reported to (do shell script shellPath & DL_Use_YTDLP & " " & YTDL_credentials & " --list-formats " & URL_user_entered)
	
	-- Need to remove extraneous spaces so that getting text items works
	...
	set download_filename_fixed to replace_chars(download_filename_formats, {"'", linefeed}, "")
	
	set accViewWidth to 500
	set set_max_width to 0 -- Width of dialog is set to maximum width of checkboxes and labels
	set theButtonSkipLabel to localized string "Skip" in bundle file path_to_MacYTDL from table "MacYTDL"
	set {theButtons, minWidth} to create buttons {theButtonCancelLabel, theButtonSkipLabel, theButtonDownloadLabel} button keys {".", "k", "d"} default button 3
	set {theformatsRule, theTop} to create rule 10 rule width accViewWidth
	...
	set {fmerge_formats_Checkbox, theTop, theWidth} to create checkbox theMergeCheckboxLabel left inset 5 bottom (theTop + 15) max width 300 with initial state
	set theTop to theTop + 10
	...
	set full_ID_list to {}
	set all_format_Checkboxes to {}
	repeat (numParas - z) times
		set end of full_ID_list to ""
	end repeat
	
	-- Parse each paragraph for desired data - work up from bottom of the list because DTP shows controls from bottom up
	set AppleScript's text item delimiters to " "
	repeat with x from -1 to (-(numParas - z)) by -1
		...
		if length of file_id_test is greater than 12 then
			set file_id to text 1 thru 9 of file_id_test as string
			set spacer1 to tab
			--set file_id to tab
		else
			set file_id to file_id_test
			set spacer1 to tab & tab
		end if
		
		-- Get file extension
		set file_ext to text item 2 of full_file_format
		if length of file_ext is 3 then
			set spacer2 to tab & tab
		else
			set spacer2 to tab
		end if
		...
		-- Get file size if present then video and audio codecs
		set file_size to ""
		...
		if file_size_present is true then
			...
			if test2_for_size is "~" or test2_for_size is "≈" then -- Some layouts have a space between "~" or "≈" and file size - 29/8/23 added "≈" v1.25 here and below
				...
				if length of file_res is greater than 7 then
					set spacer3 to tab
				else
					set spacer3 to tab & tab
				end if
			...
			-- Get video codec
			if text item 9 of full_file_format is "|" then -- YouTube video-only
			...
			else if text item 9 of full_file_format is not "storyboard" and text item 11 of full_file_format is "|" and file_res is not "Audio only" then -- YT both codecs with "~"
			...
			else if text item 8 of full_file_format is "|" then
			...
			if file_vcodec is not (tab & "Storyboard") and full_file_format does not contain "Default" and full_file_format contains "Video" then -- For YouTube and 7Plus - 30/8/23 v1.25 - added check for default + for SBS added test for Video
				if text item 11 of full_file_format is "video" or text item 12 of full_file_format is "video" then
					set file_acodec to "Video only"
				end if
			end if
			set spacer4 to ""
			if file_acodec is not "" and file_vcodec is not "" then set spacer4 to ", "
			set spacer5 to ""
			if length of file_size is greater than 7 then
				set spacer5 to (" " & tab)
			else
				set spacer5 to tab & tab
			end if
		else
			-- Get codec details for cases which have no file size
			if text item 7 of full_file_format is "|" then
		...
		set fileformat_item to (file_id & spacer1 & file_ext & spacer2 & file_res & spacer3 & file_size & spacer5 & file_vcodec & spacer4 & file_acodec)
		set {file_format_Checkbox, theTop, theWidth} to create checkbox fileformat_item left inset 5 bottom (theTop + 2) max width accViewWidth - 5
		set end of all_format_Checkboxes to file_format_Checkbox
		if set_max_width < theWidth then set set_max_width to (theWidth + 5)
		
	end repeat
	set AppleScript's text item delimiters to ""
	
	-- set icon_top to theTop
	set format_chooser_headings to "ID" & tab & "   Extension" & tab & "Resolution" & tab & "File size" & tab & tab & "Codecs"
	set {headings, theTop, theWidth} to create label format_chooser_headings left inset 30 bottom (theTop + 20) max width accViewWidth - 30
	if set_max_width < theWidth then set set_max_width to (theWidth + 20)
	
	set instructions_text to localized string "Select which formats you wish to download then click on Download. You can skip choosing formats or cancel the download." in bundle file path_to_MacYTDL from table "MacYTDL"
	set {instructionsLabel, theTop} to create label instructions_text left inset 75 bottom (theTop + 10) max width set_max_width - 75
	
	set {fileNameLabel, theTop} to create label "Dowloading file: " & "\"" & download_filename_fixed & "\"" left inset 75 bottom (theTop + 15) max width set_max_width - 75
	...
	set theFormatsDiagPromptLabel to localized string "MacYTDL – Choose Formats" in bundle file path_to_MacYTDL from table "MacYTDL"
	set {promptLabel, theTop} to create label theFormatsDiagPromptLabel left inset 0 bottom (theTop + 10) max width set_max_width aligns center aligned with bold type
	
	set formats_chooser_allControls to {theformatsRule, fmerge_formats_Checkbox, headings, instructionsLabel, fileNameLabel, promptLabel, MacYTDL_icon} & all_format_Checkboxes
	set {formats_button_returned, formats_button_number_returned, formats_controls_results} to display enhanced window diag_Title buttons theButtons acc view width set_max_width acc view height theTop acc view controls formats_chooser_allControls
	
	-- Need to iterate through all the checkboxes results - in reverse order because DTP shows controls from bottom of window up - for YT-DLP, "+" indicates merge; "," indicates keep separate
	if formats_button_number_returned is 3 then
		set controls_results_trimmed to items 4 thru end of formats_controls_results
		set controls_results_trimmed to reverse of controls_results_trimmed
		set file_formats_selected to ""
		set user_merge_formats_choice to item 2 of formats_controls_results
		if user_merge_formats_choice is true then
		...
		-- form up the list of format user wants
		set count_choices to 0
		repeat with y from 1 to count of controls_results_trimmed
			if item y of controls_results_trimmed is true then
				set count_choices to count_choices + 1
				if count_choices is 1 then
					set file_formats_selected to item y of full_ID_list
				else
					set file_formats_selected to file_formats_selected & merger_choice & item y of full_ID_list
				end if
			end if
		end repeat
		if count_choices is greater than 2 and merger_choice is "+" then
			...
			set manyFormatsButton to button returned of (display dialog (theFormatsDiagPromptLabel & return & return & theFormatsChoiceLabel & ": " & file_formats_selected & return & return & theManyFormatsLabel) with title diag_Title buttons {theButtonReturnLabel, theChooseLabel, theIgnoreLabel} default button 3 with icon file MacYTDL_custom_icon_file giving up after 600)
			...
			else if manyFormatsButton is theChooseLabel then
				get_formats_list(URL_user_entered, diag_Title, theButtonCancelLabel, theButtonDownloadLabel, X_position, screen_height, MacYTDL_custom_icon_file, MacYTDL_custom_icon_file_posix, theButtonReturnLabel, skip_Main_dialog, path_to_MacYTDL, DL_Use_YTDLP, shellPath)
				-- The recursion loops out here if user cancels 2nd instance of the Chooser
				if skip_Main_dialog is true and formats_button_number_returned is 1 then error number -128 -- This is relevant when user has come from Auto-download - need to just quit the applet
				set branch_execution to "Download"
				return branch_execution
			else if manyFormatsButton is theIgnoreLabel then
				set branch_execution to "Download"
				return branch_execution & " " & file_formats_selected & " " & add_to_output_template
			end if
		end if
		if file_formats_selected is "" then
			...
			if noFormatsButton is theButtonReturnLabel then
				...
				-- Skip the chooser and go back to calling point in download_video
				...
				if skip_Main_dialog is true and formats_button_number_returned is 1 then error number -128 -- This is relevant when user has come from Auto-download - need to just quit the applet
				set branch_execution to "Download"
				-- file_formats_selected & add_to_output_template are global so that their values can be passed out of the recursion
				return branch_execution & " " & file_formats_selected & " " & add_to_output_template
			end if
		end if
		set branch_execution to "Download"
		return branch_execution & " " & file_formats_selected & " " & add_to_output_template
	else if formats_button_number_returned is 2 then
		-- Skip the chooser and go back to calling point in download_video
		set branch_execution to "Skip"
		return branch_execution
	else
...
on Get_SBS_Episodes(URL_user_entered, diag_Title, theButtonOKLabel, theButtonCancelLabel, theButtonDownloadLabel, X_position, screen_height, MacYTDL_custom_icon_file, MacYTDL_custom_icon_file_posix, theButtonReturnLabel, skip_Main_dialog, path_to_MacYTDL)
	...
	-- Options for text strings that occur in two kinds of SBS show page found so far immediately before the Season number (usually "Season 1")
	-- FROM  \"><span class=\"MuiTab-wrapper\"><h2 class=\"MuiTypography-root MuiTypography-h2\">
	-- TO    </h2></span><span class=\"jss14
	
	-- These searches stopped working on some SBS shows - 6/4/23
	...
	-- These text clips are no longer in ALL show pages - so far found one for which it doesn't work
	set start_SBS_seasonnumber to (offset of "\"><span class=\"MuiTab-wrapper\"><h2 class=\"MuiTypography-root MuiTypography-h2\">" in SBS_show_page) + 86
	-- set end_SBS_seasonnumber to (offset of "</h2></span><span class=\"jss14" in SBS_show_page) - 1  -- <<== edited on 16/8/23
	set end_SBS_seasonnumber to (offset of "</h2></span><span class=\"jss" in SBS_show_page) - 1
	...
	set {MacYTDL_icon, theTop} to create image view MacYTDL_custom_icon_file_posix left inset 0 bottom theInstructionsTop - 64 view width 64 view height 64 scale image scale proportionally
...
on show_settings(YTDL_subtitles, DL_Remux_original, DL_YTDL_auto_check, DL_STEmbed, DL_audio_only, YTDL_description, DL_Limit_Rate, DL_over_writes, DL_Thumbnail_Write, DL_verbose, DL_Thumbnail_Embed, DL_Add_Metadata, DL_Use_Proxy, DL_Use_Cookies, DL_Use_Custom_Template, DL_Use_Custom_Settings, remux_format_choice, DL_TimeStamps, DL_Use_YTDLP, YTDL_version, folder_chosen, theButtonQuitLabel, theButtonCancelLabel, theButtonDownloadLabel, DL_Show_Settings, MacYTDL_prefs_file, MacYTDL_custom_icon_file_posix, diag_Title)
	...
	if DL_formats_list is true then
		set MDDL_DL_formats_list to "Yes"
	else
		set MDDL_DL_formats_list to "No"
	end if
	if DL_QT_Compat is true then
		set MDDL_DL_QT_Compat to "Yes"
	else
		set MDDL_DL_QT_Compat to "No"
	end if
