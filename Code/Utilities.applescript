...
on check_ytdl_installed(usr_bin_folder, diag_Title, youtubedl_file, ytdlp_file, theButtonQuitLabel, theButtonYesLabel, path_to_MacYTDL, theButtonOKLabel, resourcesPath, show_yt_dlp, MacYTDL_custom_icon_file)
	...
	try
		...
		--		set YTDL_ytdlp_version to (do shell script ytdlp_file_install & " --version") & " ytdlp"  <= v1.29 - " ytdlp" was passed to main.scpt to flag whether YTDL or yt-dlp were installed - no longer needed as yt-dlp is default install
...
-- v1.29 - Handler for checking user's settings file - called by main_dialog() after confirming that file exists and by utilities() when user restores an old settings file - If most recent change to settings not found, need to update settings file
