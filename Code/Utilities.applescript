...
-- If version of DTP library is old, replace with new
...
on install_ffmpeg_ffprobe(theButtonOKLabel, diag_Title, path_to_MacYTDL, usr_bin_folder, resourcesPath, MacYTDL_custom_icon_file, user_on_old_os, user_system_arch)
	if user_system_arch is "Intel" then
		...
		install_ffmpeg_ffprobe_arm(theButtonOKLabel, diag_Title, path_to_MacYTDL, usr_bin_folder, resourcesPath, MacYTDL_custom_icon_file, user_on_old_os, user_system_arch)
	end if
...
on Get_SBS_Episodes(URL_user_entered, diag_Title, theButtonOKLabel, theButtonCancelLabel, theButtonDownloadLabel, X_position, screen_height, MacYTDL_custom_icon_file, MacYTDL_custom_icon_file_posix, theButtonReturnLabel, skip_Main_dialog, path_to_MacYTDL)
	...
	-- Get name of the show - using web page to ensure what is shown is same as what user sees - search for:  ","name":"  and  ","url":"https://www.sbs.com.au/ondemand/program/
	set start_SBS_show_name to (offset of "\",\"name\":\"" in SBS_show_page) + 10
	-- In v1.21.2, added text to search for end_SBS_show_name
	set end_SBS_show_name to (offset of "\",\"url\":\"https://www.sbs.com.au/ondemand/program/" in SBS_show_page) - 1
	...
	-- Populate the lists of names and URLs - Repeat for each occurrence of an episode found in the show page - first text item is all text before first occurrence - need to find and replace special character codes for ampersand, apostrophe and comma
