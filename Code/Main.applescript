...
on download_video(shellPath, path_to_MacYTDL, MacYTDL_custom_icon_file, MacYTDL_custom_icon_file_posix, screen_width, screen_height, YTDL_simulate_file, URL_user_entered, URL_user_entered_from_auto_download, folder_chosen, diag_Title, DL_batch_status, remux_format_choice, subtitles_choice, YTDL_credentials, YTDL_subtitles, YTDL_STEmbed, YTDL_format_pref, YTDL_format, YTDL_remux_format, YTDL_Remux_original, YTDL_description, YTDL_audio_only, YTDL_audio_codec, YTDL_over_writes, YTDL_Thumbnail_Write, YTDL_Thumbnail_Embed, YTDL_metadata, YTDL_limit_rate_value, YTDL_verbose, YTDL_TimeStamps, YTDL_Use_Proxy, YTDL_Use_Cookies, YTDL_Custom_Settings, YTDL_Custom_Template, YTDL_no_part, skip_Main_dialog, theButtonOKLabel, theButtonCancelLabel, theButtonDownloadLabel, theButtonReturnLabel, theButtonQuitLabel, theButtonContinueLabel, YTDL_QT_Compat, DL_Use_YTDLP, theBestLabel)
	...
	-- *******************************************************************************************************************************************************
	-- v1.24 - Added workaround for SBS bug in YT-DLP
	...
	-- *******************************************************************************************************************************************************
	-- v1.24 - Exclude SBS workaround URLs from warning advices and from set_File_Names() - BUT, exiting from SBS Chooser doesn't flag workaround
...
on set_File_Names(shellPath, YTDL_simulate_log, URL_user_entered, ABC_show_name, SBS_show_name, DL_Use_YTDLP)
	...
	if ABC_show_name is "" and SBS_show_name is "" then -- not an ABC or SBS show page
		...
		if number_ABC_SBS_episodes is 1 then
			...
			-- Setting download_filename_new and download_filename to SBS_show_name as can't do a simulate to get file name – with multiple SBS downloads, file names will be distinguished by the "autonumber" output template element
...
on set_settings()
	...
	if settings_button_number_returned is 2 then -- Save Settings
		...
		-- set settings_audiocodec_35 to item 35 of settings_controls_results -- <= Audio codec field label
		set settings_description_choice to item 36 of settings_controls_results -- <= Description choice
		-- set settings_choice_37 to item 37 of settings_controls_results -- <= The Admin rule
		set settings_format_choice to item 38 of settings_controls_results -- <= File format choice
		-- set settings_choice_39 to item 39 of settings_controls_results -- <= The Format popup label
		set settings_folder_choice to item 40 of settings_controls_results -- <= The download path choice
		-- set settings_choice_41 to item 41 of settings_controls_results -- <= The Path label
		-- set settings_choice_42 to item 42 of settings_controls_results -- <= The MacYTDL icon
		-- set settings_choice_43 to item 43 of settings_controls_results -- <= Contains the "About" text
...
on add_To_Batch(URL_user_entered, download_filename_new, YTDL_remux_format)
	...
	-- After adding to batch, reset URLs so text box is blank and old URL not used again, ABC & SBS show name and number_ABC_SBS_episodes so that correct file name is used for next download
	set URL_user_entered to ""
	set ABC_show_name to ""
	set SBS_show_name to ""
	set SBS_show_URLs to ""
