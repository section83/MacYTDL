...
-- Set variables to contain user's macOS version - for Utilities, Settings and yt-dlp/YTDL install - check if user is on macOS 12.3+ for Utilities dialog - Check if user on <12.2.1 and so will have expired certificates preventing FFmpeg dowload
...
if YTDL_exists is false and ytdlp_exists is false and DTP_exists is false and ffmpeg_exists is false and ffprobe_exists is false and prefs_exists is false then
	...
	if components_install_answ is theButtonYesLabel then
		...
		run_Utilities_handlers's set_preferences(old_version_prefs, diag_Title, theButtonNoLabel, theButtonYesLabel, MacYTDL_prefs_file, MacYTDL_version, MacYTDL_date, MacYTDL_preferences_path, path_to_MacYTDL, X_position, Y_position, theBestLabel, theDefaultLabel, resourcesPath, show_yt_dlp, YTDL_version, MacYTDL_custom_icon_file)
		set prefs_exists to true
		delay 1
		run_Utilities_handlers's install_DTP(DTP_file, path_to_MacYTDL, resourcesPath)
		set DTP_exists to true
		delay 1
		run_Utilities_handlers's check_ffmpeg_installed(theButtonOKLabel, diag_Title, path_to_MacYTDL, usr_bin_folder, ffmpeg_exists, ffprobe_exists, resourcesPath, MacYTDL_custom_icon_file, user_on_old_os)
		set ffprobe_exists to true
		set ffmpeg_exists to true
...
run_Utilities_handlers's check_settings(MacYTDL_prefs_file, old_version_prefs, diag_Title, theButtonNoLabel, theButtonYesLabel, MacYTDL_version, MacYTDL_date, MacYTDL_preferences_path, path_to_MacYTDL, X_position, Y_position, theBestLabel, theDefaultLabel, resourcesPath, show_yt_dlp, YTDL_version)
...
run_Utilities_handlers's check_for_v1_21_settings(MacYTDL_prefs_file, DL_Use_YTDLP, MacYTDL_preferences_path, youtubedl_file, ytdlp_file)
...
on main_dialog()
	...
	if DL_Use_YTDLP is "yt-dlp" then
		set {main_thePopUp_FileFormat, main_formatlabel, theTop, popupLeft} to create labeled popup {"No remux", "mp4", "mkv", "flv", "webm", "mov", "avi", "ogg"} left inset accViewInset - 5 bottom (theTop + 5) popup width 100 max width 200 label text theLabelledPopUpRemuxFileformat popup left accViewInset - 5 initial choice DL_Remux_format
	else
		set {main_thePopUp_FileFormat, main_formatlabel, theTop, popupLeft} to create labeled popup {"No remux", "mp4", "mkv", "flv", "webm", "avi", "ogg"} left inset accViewInset - 5 bottom (theTop + 5) popup width 100 max width 200 label text theLabelledPopUpRemuxFileformat popup left accViewInset - 5 initial choice DL_Remux_format
	end if
...
--
---------------------------------------------------------------------------------------------
-- Following call is a backup containing monitor_dialog_position
...
on download_video(shellPath, path_to_MacYTDL, MacYTDL_custom_icon_file, MacYTDL_custom_icon_file_posix, screen_width, screen_height, YTDL_simulate_file, URL_user_entered, URL_user_entered_from_auto_download, folder_chosen, diag_Title, DL_batch_status, remux_format_choice, subtitles_choice, YTDL_credentials, YTDL_subtitles, YTDL_STEmbed, YTDL_format_pref, YTDL_format, YTDL_remux_format, YTDL_Remux_original, YTDL_description, YTDL_audio_only, YTDL_audio_codec, YTDL_over_writes, YTDL_Thumbnail_Write, YTDL_Thumbnail_Embed, YTDL_metadata, YTDL_limit_rate_value, YTDL_verbose, YTDL_TimeStamps, YTDL_Use_Proxy, YTDL_Use_Cookies, YTDL_Custom_Settings, YTDL_Custom_Template, YTDL_no_part, skip_Main_dialog, theButtonOKLabel, theButtonCancelLabel, theButtonDownloadLabel, theButtonReturnLabel, theButtonQuitLabel)
	...
	-- Fourth, test whether the URL is an iView or OnDemand page not supported by yt-dlp/youtube-dl
	if URL_user_entered contains "https://iview.abc.net.au/category/" or URL_user_entered contains "https://iview.abc.net.au/collection/" then
		set theURLWarningiViewCategoryLabel to localized string "This is an iView Category page from which MacYTDL cannot download videos. Try an individual show." from table "MacYTDL"
		...
		main_dialog()
	else if URL_user_entered is "https://www.sbs.com.au/ondemand/" or URL_user_entered contains "https://www.sbs.com.au/ondemand/sport" or URL_user_entered contains "https://www.sbs.com.au/ondemand/movies" or URL_user_entered contains "https://www.sbs.com.au/ondemand/live" then
	...
	-- Sixth, is the URL a YouTube channel - if so warn user it may contain a great many videos and take hours to work
	if URL_user_entered_clean contains "https://www.youtube.com/c/" or URL_user_entered_clean contains "https://www.youtube.com/channel/" or URL_user_entered_clean contains "https://www.youtube.com/user/" then
	...
	if URL_user_entered_clean contains "&index=" and URL_user_entered_clean contains "youtu" then
		...
		-- Does the URL point to a  playlist ? If so, warn user to wait while the playlist is checked
		...
		set alerterPiD to do shell script resourcesPath & "alerter -message " & playListAlertMessage & " -title " & playListAlertTitle & " -subtitle " & playListAlertSubtitle & " -timeout 10 -sender com.apple.script.id.MacYTDL -actions " & playListAlertActionLabel & " > /dev/null 2> /dev/null &  echo $!"
		try
	...
	if (length of YTDL_simulate_response is greater than 190) and ((count of paragraphs in YTDL_simulate_response) is less than 3) then
		if YTDL_Custom_Template is not "" then
			set theFileNameTooLongLabel to localized string "With your custom file name output template, the total length of the response file is too long. Would you like to exclude your custom template then continue or return to Main ?" from table "MacYTDL"
	...
	-- Is the URL from an ABC or SBS Show Page ? - If so, get the user to choose which episodes to download
	if URL_user_entered_clean contains "iview.abc.net.au/show/" then
		...
		set theURLWarning10playLabel to localized string "This is a 10 play Show page from which MacYTDL cannot download videos. Try an individual episode." from table "MacYTDL"
		...
		if playlist_Name is not "" then
			...
			set theURLErrorTextLabel4 to localized string theURLErrorTextLabelString & "'" & playlist_Name & "':" from table "MacYTDL"
		else
		...
		if skip_Main_dialog is true then
			...
			set quit_or_return to button returned of (display dialog theURLErrorTextLabel1 & theURLErrorTextLabel4 & return & return & URL_user_entered & return & return & theURLErrorTextLabel2 & return & return & YTDL_simulate_response & return & theURLErrorTextLabel3 buttons {theButtonOKLabel, theButtonDownloadLabel} default button 2 cancel button 1 with title diag_Title with icon file MacYTDL_custom_icon_file giving up after 600)
			if quit_or_return is theButtonOKLabel then
				return
			else if quit_or_return is theButtonDownloadLabel then
			...
			set quit_or_return to button returned of (display dialog theURLErrorTextLabel1 & theURLErrorTextLabel4 & return & return & URL_user_entered & return & return & theURLErrorTextLabel2 & return & return & YTDL_simulate_response & return & theURLErrorTextLabel3 buttons {theButtonQuitLabel, theButtonOKLabel, theButtonDownloadLabel} default button 2 cancel button 1 with title diag_Title with icon file MacYTDL_custom_icon_file giving up after 600)
			if quit_or_return is theButtonOKLabel then
				main_dialog()
			else if quit_or_return is theButtonDownloadLabel then
				-- User wants to try to download ! Processing just continues from here down
	...
	-- Sixth - check whether URL points to a live stream - add "no-part" so that file is playable - exclude playlists as by definition they can't be live streams
	-- SBS streams are not identified by youtube-dl - most seem to have the words live stream in file name or URL
	if playlist_Name is "" then
		if URL_user_entered contains "sbs.com.au/ondemand" and (URL_user_entered contains "-live-stream" or YTDL_simulate_response contains "Live Stream") then
	...
	repeat with aPara in (paragraphs of YTDL_simulate_response)
		...
		if aPara contains "WARNING:" and aPara does not contain "Falling back on generic information" and aPara does not contain "Ignoring subtitle tracks found in the HLS manifest" then
	...
	if simulate_warnings is not "" then
		...
		if warning_quit_or_continue is theButtonContinueLabel then -- <= Ignore warning - try DL - get filename from last paragraph of simulate file
			set_File_Names(shellPath, YTDL_simulate_response, URL_user_entered, ABC_show_name, SBS_show_name)
	...
	set ytdl_settings to quoted form of (" --ignore-errors --newline " & YTDL_subtitles & YTDL_STEmbed & YTDL_credentials & YTDL_format & YTDL_remux_format & YTDL_Remux_original & YTDL_description & YTDL_audio_only & YTDL_audio_codec & YTDL_over_writes & YTDL_Thumbnail_Write & YTDL_Thumbnail_Embed & YTDL_metadata & YTDL_limit_rate_value & YTDL_verbose & YTDL_Use_Proxy & YTDL_Use_Cookies & YTDL_no_playlist & YTDL_no_part & YTDL_Custom_Settings & YTDL_output_template & " ")
				...
				if length_each_filename is not 0 then
					...
					if search_for_download is not {} then
						...
						if overwrite_continue_choice is theABCShowExistsButtonOverwriteLabel then
							...
							set ytdl_settings to quoted form of (" --ignore-errors --newline " & YTDL_subtitles & YTDL_STEmbed & YTDL_credentials & YTDL_format & YTDL_remux_format & YTDL_Remux_original & YTDL_description & YTDL_audio_only & YTDL_audio_codec & YTDL_metadata & YTDL_limit_rate_value & YTDL_verbose & YTDL_Use_Proxy & YTDL_Use_Cookies & YTDL_no_playlist & YTDL_no_part & YTDL_Custom_Settings & YTDL_output_template & " ")
							...
							set ytdl_settings to quoted form of (" --ignore-errors --newline " & YTDL_subtitles & YTDL_STEmbed & YTDL_credentials & YTDL_format & YTDL_remux_format & YTDL_Remux_original & YTDL_description & YTDL_audio_only & YTDL_audio_codec & YTDL_over_writes & YTDL_metadata & YTDL_limit_rate_value & YTDL_verbose & YTDL_Use_Proxy & YTDL_Use_Cookies & YTDL_no_playlist & YTDL_no_part & YTDL_Custom_Settings & YTDL_output_template_new & " ")
	...
	-- Get number of current downloads underway - send to monitor.scpt for positioning monitor dialogs	
...
--
------------------------------------------------------------
on set_File_Names(shellPath, YTDL_simulate_response, URL_user_entered, ABC_show_name, SBS_show_name)
...
on set_settings()
	...
	if DL_Use_YTDLP is "yt-dlp" then
		set {settings_thePopUp_RemuxFormat, settings_remuxlabel, theTop} to create labeled popup {"No remux", "mp4", "mkv", "flv", "webm", "mov", "avi", "ogg"} left inset 70 bottom (theTop + 5) popup width 100 max width 200 label text theLabeledPopupRemuxFormatLabel popup left 70 initial choice DL_Remux_format
	else
		set {settings_thePopUp_RemuxFormat, settings_remuxlabel, theTop} to create labeled popup {"No remux", "mp4", "mkv", "flv", "webm", "avi", "ogg"} left inset 70 bottom (theTop + 5) popup width 100 max width 200 label text theLabeledPopupRemuxFormatLabel popup left 70 initial choice DL_Remux_format
	end if
	...
	set {settings_button_returned, settings_button_number_returned, settings_controls_results} to display enhanced window diag_Title buttons theButtons acc view width calculatedAccViewWidth acc view height theTop acc view controls settings_allControls initial position window_Position
	...
	if settings_button_number_returned is 2 then -- Save Settings
		...
		set settings_use_custom_settings_choice to item 4 of settings_controls_results -- <= Use custom settings choice
		set settings_custom_settings_choice to item 5 of settings_controls_results -- <= The custom settings to use
		set settings_use_custom_template_choice to item 6 of settings_controls_results -- <= Use custom template choice
		set settings_custom_template_choice to item 7 of settings_controls_results -- <= The custom template to use
		set settings_use_cookies_choice to item 8 of settings_controls_results -- <= Use proxy choice
		set settings_cookies_location_choice to item 9 of settings_controls_results -- <= The proxy URL
		set settings_use_proxy_choice to item 10 of settings_controls_results -- <= Use proxy choice
		set settings_proxy_URL_choice to item 11 of settings_controls_results -- <= The proxy URL
		set settings_limit_rate_choice to item 12 of settings_controls_results -- <= Limit rate choice
		set settings_limit_rate_value_choice to item 13 of settings_controls_results -- <= Limit rate value choice
		set settings_original_choice to item 14 of settings_controls_results -- <= Keep original after remux choice
		set settings_remux_format_choice to item 15 of settings_controls_results -- <= Remux format choice
		-- set settings_choice_16 to item 16 of settings_controls_results -- <= The Remux format popup label
		set settings_metadata_choice to item 17 of settings_controls_results -- <= Add metadata choice
		set settings_timestamps_choice to item 18 of settings_controls_results -- <= Add timestamps choice
		set settings_verbose_choice to item 19 of settings_controls_results -- <= Verbose choice
		set settings_thumb_embed_choice to item 20 of settings_controls_results -- <= Embed Thumbnails choice
		set settings_thumb_write_choice to item 21 of settings_controls_results -- <= Write Thumbnails choice
		set settings_autoST_choice to item 22 of settings_controls_results -- <= Auto-gen subtitles choice
		set settings_subtitlesformat_choice to item 23 of settings_controls_results -- <= Subtitles format choice
		-- set settings_STFormatlabel_choice to item 24 of settings_controls_results -- <= Subtitles format popup label
		set settings_subtitleslanguage_choice to item 25 of settings_controls_results -- <= Subtitles language choice
		-- set settings_subtitleslanguage_26 to item 26 of settings_controls_results -- <= Subtitles language field label
		set settings_stembed_choice to item 27 of settings_controls_results -- <= Embed subtitles choice
		set settings_subtitles_choice to item 28 of settings_controls_results -- <= Subtitles choice
		set settings_YTDL_auto_choice to item 29 of settings_controls_results -- <= Auto check YTDL version on startup choice
		set settings_audio_only_choice to item 30 of settings_controls_results -- <= Audio only choice
		set settings_audio_codec_choice to item 31 of settings_controls_results -- <= Audio codec choice
		-- set settings_audiocodec_32 to item 32 of settings_controls_results -- <= Audio codec field label
		set settings_description_choice to item 33 of settings_controls_results -- <= Description choice
		...
		-- set settings_choice_39 to item 39 of settings_controls_results -- <= Contains the "About" text
	...
	if FFmpeg_page is "" then
		...
		if ffmpeg_version_check is not equal to ffmpeg_version then
			...
			if ffmpeg_install_answ is theButtonYesLabel then
				...
				do shell script "curl -L " & ffmpeg_site & "ffmpeg-" & ffmpeg_version_check & ".zip" & ignore_certifcates & " -o " & ffmpeg_download_file with administrator privileges
				try
				...
				on error errStr number errorNumber
					if errorNumber is -128 then
						...
						set theFFmpegUpdateProblemTextLabel1 to localized string "There was a problem with installing FFmpeg. This was the error message: " from table "MacYTDL"
						set theFFmpegUpdateProblemTextLabel2 to localized string "You can run MacYTDL and change settings but downloads will not work until FFmpeg is installed. When you next start MacYTDL, it will try again to install FFmpeg." from table "MacYTDL"
						display dialog theFFmpegUpdateProblemTextLabel1 & errorNumber & " " & errStr & return & return & theFFmpegUpdateProblemTextLabel2 buttons {theButtonOKLabel} default button 1 with title diag_Title with icon file MacYTDL_custom_icon_file giving up after 600
					end if
				end try
...
on utilities()
	...
	-- Set Settings, youtube-dl/yt-dlp and FFmpeg version installed text - to show in Utilities dialog
	...
	if (user_on_123 is true and DL_Use_YTDLP is "yt-dlp") or user_on_old_os is true then
		set {utilities_theCheckbox_Switch_Scripts, theTop} to create label " " left inset accViewInset + 5 bottom (theTop - 17) max width minWidth - 100 aligns left with multiline
	else
	...
	if utilities_button_number_returned is 5 then -- Start
		...
		set utilities_Return_Defaults_choice to item 6 of utilities_controls_results -- <= Return to default settings choice
		set utilities_Save_Settings_choice to item 7 of utilities_controls_results -- <= Save current settings choice
		set utilities_Restore_Settings_choice to item 8 of utilities_controls_results -- <= Restore saved settings choice
		set utilities_Switch_choice to item 9 of utilities_controls_results -- <= Switch downloaders choice
		set utilities_YTDL_webpage_choice to item 10 of utilities_controls_results -- <= Show YTDL/yt-dlp web page choice
		set utilities_YTDL_check_choice to item 11 of utilities_controls_results -- <= Check YTDL/yt-dlp version choice
		set utilities_DL_folder_choice to item 12 of utilities_controls_results -- <= Open download folder choice
		set utilities_log_folder_choice to item 13 of utilities_controls_results -- <= Open log folder choice
		--set utilities_choice_14 to item 14 of utilities_controls_results -- <= Missing value [the icon]
		--set utilities_choice_15 to item 15 of utilities_controls_results -- <= Contains the "Instructions" text
		--set utilities_choice_16 to item 16 of utilities_controls_results -- <= Contains the "Utilities" heading
		...
		if utilities_Return_Defaults_choice is true then
			...
			if Really_Return_To_Defaults is "Yes" then
				run_Utilities_handlers's set_preferences(old_version_prefs, diag_Title, theButtonNoLabel, theButtonYesLabel, MacYTDL_prefs_file, MacYTDL_version, MacYTDL_date, MacYTDL_preferences_path, path_to_MacYTDL, X_position, Y_position, theBestLabel, theDefaultLabel, resourcesPath, show_yt_dlp, YTDL_version, MacYTDL_custom_icon_file)
			end if
		end if
...
on download_batch(folder_chosen, remux_format_choice, subtitles_choice, YTDL_credentials, YTDL_subtitles, YTDL_STEmbed, YTDL_format, YTDL_remux_format, YTDL_Remux_original, YTDL_description, YTDL_audio_only, YTDL_audio_codec, YTDL_over_writes, YTDL_Thumbnail_Write, YTDL_Thumbnail_Embed, YTDL_metadata, YTDL_limit_rate_value, YTDL_verbose, YTDL_TimeStamps, YTDL_Use_Proxy, YTDL_Use_Cookies, YTDL_Custom_Settings, YTDL_Custom_Template, YTDL_no_part)
	...
	-- "Bennett:Users:macytdl:MacYTDL:MacYTDL:MacYTDL.app:Contents:Resources:macytdl.icns"
	...
	set ytdl_settings to quoted form of (" --ignore-errors --newline " & YTDL_subtitles & YTDL_STEmbed & YTDL_credentials & YTDL_format & YTDL_remux_format & YTDL_Remux_original & YTDL_description & YTDL_audio_only & YTDL_audio_codec & YTDL_over_writes & YTDL_Thumbnail_Write & YTDL_Thumbnail_Embed & YTDL_metadata & YTDL_limit_rate_value & YTDL_verbose & YTDL_Use_Proxy & YTDL_Use_Cookies & YTDL_no_part & YTDL_Custom_Settings & YTDL_output_template & " --batch-file " & YTDL_batch_file)
	
	set my_params to quoted form of folder_chosen & " " & MacYTDL_preferences_path & " " & YTDL_TimeStamps_quoted & " " & ytdl_settings & " " & URL_user_entered & " " & YTDL_response_file & " " & download_filename & " " & download_filename_new & " " & MacYTDL_custom_icon_file_posix & " " & monitor_dialog_position & " " & YTDL_simulate_response & " " & diag_Title_quoted & " " & is_Livestream_Flag & " " & screen_width & " " & screen_height & " " & DL_Use_YTDLP
