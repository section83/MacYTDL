...
on main_dialog()
	...
	-- Need to work through why I put this code here - deary me, a lack of documentation - originally intended to fix a translation issue but, does it causing one ?
	...
	-- v1.28 - Found that FFmpeg added "-codec copy" to ST converter which then failed -- found that -codec copy was not needed anyway -- youtube-dl doesn't have remux-video
	...
	if credentials_choice is true then
		run_Utilities_handlers's get_YTDL_credentials(theButtonReturnLabel, theButtonOKLabel, MacYTDL_custom_icon_file_posix, diag_Title)
	end if
...
on download_video(shellPath, path_to_MacYTDL, MacYTDL_custom_icon_file, MacYTDL_custom_icon_file_posix, screen_width, screen_height, YTDL_simulate_file, URL_user_entered, URL_user_entered_from_auto_download, folder_chosen, diag_Title, DL_batch_status, remux_format_choice, subtitles_choice, YTDL_credentials, YTDL_subtitles, YTDL_STEmbed, YTDL_format_pref, YTDL_format, YTDL_remux_format, YTDL_Remux_original, YTDL_description, YTDL_audio_only, YTDL_audio_codec, YTDL_over_writes, YTDL_Thumbnail_Write, YTDL_Thumbnail_Embed, YTDL_metadata, YTDL_limit_rate_value, YTDL_verbose, YTDL_TimeStamps, YTDL_Use_Proxy, YTDL_Use_Cookies, YTDL_Custom_Settings, YTDL_Custom_Template, YTDL_no_part, skip_Main_dialog, theButtonOKLabel, theButtonCancelLabel, theButtonDownloadLabel, theButtonReturnLabel, theButtonQuitLabel, theButtonContinueLabel, YTDL_QT_Compat, DL_Use_YTDLP, theBestLabel, YTDL_Resolution_Limit, YTDL_Use_Parts, YTDL_No_Warnings, ADL_Clear_Batch, user_on_old_os, X_position)
	...
	if URL_user_entered_clean contains "&index=" and URL_user_entered_clean contains "youtu" then
		...
		if DL_Parallel is false then
			...
			set alerterPiD to do shell script quoted form of (resourcesPath & "alerter") & " -message " & playListAlertMessage & " -title " & playListAlertTitle & " -subtitle " & playListAlertSubtitle & " -sender com.apple.script.id.MacYTDL -actions " & playListAlertActionLabel & " -timeout 20 > /dev/null 2> /dev/null & echo $!"
			try
				do shell script shellPath & "cd " & quoted form of downloadsFolder_Path & " ; export LC_CTYPE=UTF-8 ; " & DL_Use_YTDLP & " --print '%(playlist_title)s##%(webpage_url)s##%(title)s.%(ext)s' " & DL_Playlist_Items_Spec & YTDL_No_Warnings & YTDL_Use_Cookies & URL_user_entered_clean_quoted & " 2>&1 &>" & quoted form of YTDL_simulate_file & " ; exit 0"
				try
					do shell script "kill " & alerterPiD
				end try
			on error error_Message
	...
	if (length of YTDL_simulate_log is greater than 195) and ((count of paragraphs in YTDL_simulate_log) is less than 3) then
		...
		do shell script shellPath & "cd " & quoted form of downloadsFolder_Path & " ; export LC_CTYPE=UTF-8 ; " & DL_Use_YTDLP & " --get-filename --ignore-errors " & YTDL_no_playlist & DL_Playlist_Items_Spec & YTDL_format_pref & YTDL_credentials & YTDL_Use_Proxy & YTDL_Use_Cookies & YTDL_No_Warnings & simulate_YTDL_output_template & " " & URL_user_entered_clean_quoted & " 2>&1 &>" & quoted form of YTDL_simulate_file & " ; exit 0"
		-- Added delay as one user gets end of file errors which might be due to simulate file not being ready
		delay 1
		try
			set YTDL_simulate_log to read POSIX file YTDL_simulate_file as «class utf8»
	...
	if YTDL_simulate_log contains "requested format not available" then
		...
		if quit_or_return is theButtonReturnLabel then
			...
			do shell script shellPath & "cd " & quoted form of downloadsFolder_Path & " ; export LC_CTYPE=UTF-8 ; " & DL_Use_YTDLP & " --get-filename " & YTDL_no_playlist & DL_Playlist_Items_Spec & YTDL_credentials & YTDL_No_Warnings & URL_user_entered_clean_quoted & " " & simulate_YTDL_output_template & " > /dev/null" & " &> " & quoted form of YTDL_simulate_file
			set YTDL_format to ""
			-- Added delay as one user gets end of file errors which might be due to simulate file not being ready
			delay 1
			try
	...
	if simulate_warnings is not "" then
		...
		if warning_quit_or_continue is theButtonContinueLabel then -- <= Ignore warning - try DL - get filename from last paragraph of simulate file
			...
			set branch_execution to set_File_Names(shellPath, YTDL_simulate_log, URL_user_entered, ABC_show_name, SBS_show_name, DL_Use_YTDLP, parallel_playlist)
			if branch_execution is "Main" then return
			-- ************************************************************************************************************************************************************		
		...
		-- ************************************************************************************************************************************************************
		-- v1.29.2 - 11/5/25 - First version of repeat loop to control flow
		if URL_user_entered is "Main" then return
		-- ************************************************************************************************************************************************************		
	...
	if subtitles_choice is true or DL_YTAutoST is true then
		...
		-- ************************************************************************************************************************************************************
		-- v1.29.2 - 11/5/25 - First version of repeat loop to control flow
		if YTDL_subtitles is "Main" then return
		-- ************************************************************************************************************************************************************		
	...
	if DL_batch_status is true then
		...
		set branch_execution to add_To_Batch(URL_user_entered, download_filename, download_filename_new, YTDL_remux_format)
		if branch_execution is "Main" then return
		-- ************************************************************************************************************************************************************		
...
on set_settings()
	...
	tell application "System Events"
		if exists file MacYTDL_prefs_file then
			...
			run_Utilities_handlers's set_preferences(old_version_prefs, diag_Title, theButtonNoLabel, theButtonYesLabel, MacYTDL_prefs_file, MacYTDL_version, MacYTDL_date, MacYTDL_preferences_path, path_to_MacYTDL, X_position, Y_position, theBestLabel, theDefaultLabel, theNoRemuxLabel, resourcesPath, show_yt_dlp, MacYTDL_custom_icon_file)
			set prefs_exists to true
		end if
	end tell
	set DL_format to localized string DL_format from table "MacYTDL"
	...
	if DL_Use_YTDLP is "yt-dlp" then
		...
		set {settings_thePopUp_RemuxFormat, settings_remuxlabel, theTop} to create labeled popup {theNoRemuxLabel, "avi", "flv", "gif", "mkv", "mov", "mp4", "webm", "aiff", "flac", "m4a", "mka", "mp3", "ogg", "wav"} left inset 70 bottom (theTop + 3) popup width 100 max width 200 label text theLabeledPopupRemuxFormatLabel popup left 70 initial choice DL_Remux_format
	else
...
-- Handler to check and update yt-dlp if user wishes - called by Utilities dialog to update script and switch to yt-dlp, the auto check on startup and the Warning dialog - Not called if user has Homebrew or Nightly yt-dlp
...
on check_ytdl(show_yt_dlp)
	-- Get version of yt-dlp available from GitHub - which has a different name to what is used by MacYTDL - get legacy version for users on 10.9-10.15
...
on check_ffmpeg()
	if user_system_arch is "Intel" then
		if show_yt_dlp is "yt-dlp-legacy" then
			...
			-- ************************************************************************************************************************************************************
			-- v1.29.2 - 26/4/25 - First version of repeat loop to control flow
			set alert_text_ffmpeg to run_Utilities_handlers's check_ffmpeg_intel_OLD(ffmpeg_file, ffprobe_file, theButtonOKLabel, diag_Title, MacYTDL_custom_icon_file, theButtonNoLabel, theButtonYesLabel, usr_bin_folder, resourcesPath, user_on_old_os, path_to_MacYTDL)
			...
			-- ************************************************************************************************************************************************************
			-- v1.29.2 - 26/4/25 - First version of repeat loop to control flow
			set alert_text_ffmpeg to run_Utilities_handlers's check_ffmpeg_intel(ffmpeg_file, ffprobe_file, theButtonOKLabel, diag_Title, MacYTDL_custom_icon_file, theButtonNoLabel, theButtonYesLabel, usr_bin_folder, resourcesPath, user_on_old_os, path_to_MacYTDL)
			if alert_text_ffmpeg is "Main" then return "Main"
			return alert_text_ffmpeg
			-- ************************************************************************************************************************************************************
		...
		-- ************************************************************************************************************************************************************
		-- v1.29.2 - 26/4/25 - First version of repeat loop to control flow
		set alert_text_ffmpeg to run_Utilities_handlers's check_ffmpeg_arm(ffmpeg_file, ffprobe_file, theButtonOKLabel, diag_Title, MacYTDL_custom_icon_file, theButtonNoLabel, theButtonYesLabel, path_to_MacYTDL, usr_bin_folder, resourcesPath, user_on_old_os)
		if alert_text_ffmpeg is "Main" then return "Main"
		return alert_text_ffmpeg
		-- ************************************************************************************************************************************************************
	...
	if utilities_button_number_returned is 5 then -- Start
		...
		if utilities_YTDL_check_choice contains "build" and utilities_FFmpeg_check_choice is true then
			...
			if ffmpeg_version is "Not installed" then
				...
				-- v1.29.2 - 26/4/25 - First version of repeat loop to control flow
				--  check_ffmpeg()
				set branch_execution to check_ffmpeg()
				if branch_execution is "Main" then return
				-- ************************************************************************************************************************************************************
		...
		if utilities_Atomic_choice is true then
			if Atomic_is_installed is false then
				if DL_Use_YTDLP is "yt-dlp" then
					...
					-- ************************************************************************************************************************************************************
					-- v1.29.2 - 26/4/25 - First version of repeat loop to control flow
					if reallyWantsAP is theButtonNoLabel then return
		...
		if utilities_Service_choice is true then
			if isServiceInstalled is "No" then
				...
				if DL_auto is true then
					...
					-- ************************************************************************************************************************************************************
					-- v1.29.2 - 26/4/25 - First version of repeat loop to control flow
					if reallyWantsAPRemoved is theButtonReturnLabel then return
		...
		--		display dialog DL_Use_YTDLP & return & MacYTDL_date & return & theButtonOKLabel & return & diag_Title & return & MacYTDL_custom_icon_file_posix
...
on open_batch_processing(folder_chosen, remux_format_choice, subtitles_choice, YTDL_credentials, YTDL_subtitles, YTDL_STEmbed, YTDL_format, YTDL_remux_format, YTDL_Remux_original, YTDL_description, YTDL_audio_only, YTDL_audio_codec, YTDL_over_writes, YTDL_Thumbnail_Write, YTDL_Thumbnail_Embed, YTDL_metadata, YTDL_limit_rate_value, YTDL_verbose, YTDL_TimeStamps, YTDL_Use_Proxy, YTDL_Use_Cookies, YTDL_Custom_Settings, YTDL_Custom_Template, YTDL_no_part, YTDL_QT_Compat, DL_Use_YTDLP, YTDL_Resolution_Limit, YTDL_Use_Parts, YTDL_No_Warnings, ADL_Clear_Batch)
	...
	-- ************************************************************************************************************************************************************
	-- 1.92.2 - tally_batch() moved to utilities.scpt
	-- Start by calculating tally of URLs currently saved in the batch file	
	set batch_tally_number to run_Utilities_handlers's tally_batch(batch_file, theButtonOKLabel)
	if batch_tally_number is "Main" then return
	-- ************************************************************************************************************************************************************
	...
	if batchButtonNumberReturned is 5 then
		...
		run_Utilities_handlers's clear_batch(folder_chosen, remux_format_choice, subtitles_choice, YTDL_credentials, YTDL_subtitles, YTDL_STEmbed, YTDL_format, YTDL_remux_format, YTDL_Remux_original, YTDL_description, YTDL_audio_only, YTDL_audio_codec, YTDL_over_writes, YTDL_Thumbnail_Write, YTDL_Thumbnail_Embed, YTDL_metadata, YTDL_limit_rate_value, YTDL_verbose, YTDL_TimeStamps, YTDL_Use_Proxy, YTDL_Use_Cookies, YTDL_Custom_Settings, YTDL_Custom_Template, YTDL_no_part, YTDL_QT_Compat, DL_Use_YTDLP, YTDL_Resolution_Limit, YTDL_Use_Parts, YTDL_No_Warnings, ADL_Clear_Batch, batch_file, diag_Title, theButtonOKLabel, MacYTDL_custom_icon_file)
	else if batchButtonNumberReturned is 4 then
		run_Utilities_handlers's remove_last_from_batch(folder_chosen, remux_format_choice, subtitles_choice, YTDL_credentials, YTDL_subtitles, YTDL_STEmbed, YTDL_format, YTDL_remux_format, YTDL_Remux_original, YTDL_description, YTDL_audio_only, YTDL_audio_codec, YTDL_over_writes, YTDL_Thumbnail_Write, YTDL_Thumbnail_Embed, YTDL_metadata, YTDL_limit_rate_value, YTDL_verbose, YTDL_TimeStamps, YTDL_Use_Proxy, YTDL_Use_Cookies, YTDL_Custom_Settings, YTDL_Custom_Template, YTDL_no_part, YTDL_QT_Compat, DL_Use_YTDLP, YTDL_Resolution_Limit, YTDL_Use_Parts, YTDL_No_Warnings, ADL_Clear_Batch, batch_file, diag_Title, theButtonOKLabel, MacYTDL_custom_icon_file)
	end if
