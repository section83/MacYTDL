...
on download_video(shellPath, path_to_MacYTDL, MacYTDL_custom_icon_file, MacYTDL_custom_icon_file_posix, screen_width, screen_height, YTDL_simulate_file, URL_user_entered, URL_user_entered_from_auto_download, folder_chosen, diag_Title, DL_batch_status, remux_format_choice, subtitles_choice, YTDL_credentials, YTDL_subtitles, YTDL_STEmbed, YTDL_format_pref, YTDL_format, YTDL_remux_format, YTDL_Remux_original, YTDL_description, YTDL_audio_only, YTDL_audio_codec, YTDL_over_writes, YTDL_Thumbnail_Write, YTDL_Thumbnail_Embed, YTDL_metadata, YTDL_limit_rate_value, YTDL_verbose, YTDL_TimeStamps, YTDL_Use_Proxy, YTDL_Use_Cookies, YTDL_Custom_Settings, YTDL_Custom_Template, YTDL_no_part, skip_Main_dialog, theButtonOKLabel, theButtonCancelLabel, theButtonDownloadLabel, theButtonReturnLabel, theButtonQuitLabel, theButtonContinueLabel, YTDL_QT_Compat)
	...
	-- Sixth, is the URL a YouTube channel - if so warn user it may contain a great many videos and take hours to work - but send youtube-dl users back to Main
	...
	-- Do a simulation to get back file names and disclose any errors or warnings
	...
	-- Takes a long time when simulating channels and playlists - users are warned
	...
	if (length of YTDL_simulate_log is greater than 190) and ((count of paragraphs in YTDL_simulate_log) is less than 3) then
		if YTDL_Custom_Template is not "" then
		...
		do shell script shellPath & "cd " & quoted form of downloadsFolder_Path & " ; export LC_CTYPE=UTF-8 ; " & DL_Use_YTDLP & " --get-filename --ignore-errors " & YTDL_no_playlist & YTDL_format_pref & YTDL_credentials & YTDL_Use_Proxy & YTDL_Use_Cookies & YTDL_output_template & " " & URL_user_entered_clean_quoted & " 2>&1 &>" & YTDL_simulate_file & " ; exit 0"
		set YTDL_simulate_log to read POSIX file YTDL_simulate_file as "utf8"
	end if
	...
	if YTDL_simulate_log contains "Unsupported URL: https://7Plus.com.au" then
		...
		main_dialog()
	else if YTDL_simulate_log contains "requested format not available" then
		...
		if quit_or_return is theButtonReturnLabel then
			...
			do shell script shellPath & "cd " & quoted form of downloadsFolder_Path & " ; export LC_CTYPE=UTF-8 ; " & DL_Use_YTDLP & " --get-filename " & YTDL_no_playlist & YTDL_credentials & URL_user_entered_clean_quoted & " " & YTDL_output_template & " > /dev/null" & " &> " & YTDL_simulate_file
			set YTDL_format to ""
			set YTDL_simulate_log to read POSIX file YTDL_simulate_file as "utf8"
	...
	else if YTDL_simulate_log contains "IOError: CRC check failed" then
		...
		main_dialog()
	else if URL_user_entered_clean contains "iview.abc.net.au/show/" then
	...
	else if YTDL_simulate_log contains "Unsupported URL: https://www.sbs.com.au/ondemand" then
	...
	if playlist_Name is "" then
		-- Can't DL SBS live streams at present – need a YT-DLP extractor fix
...
on download_batch(folder_chosen, remux_format_choice, subtitles_choice, YTDL_credentials, YTDL_subtitles, YTDL_STEmbed, YTDL_format, YTDL_remux_format, YTDL_Remux_original, YTDL_description, YTDL_audio_only, YTDL_audio_codec, YTDL_over_writes, YTDL_Thumbnail_Write, YTDL_Thumbnail_Embed, YTDL_metadata, YTDL_limit_rate_value, YTDL_verbose, YTDL_TimeStamps, YTDL_Use_Proxy, YTDL_Use_Cookies, YTDL_Custom_Settings, YTDL_Custom_Template, YTDL_no_part)
	...
	set my_params to quoted form of folder_chosen & " " & quoted form of MacYTDL_preferences_path & " " & YTDL_TimeStamps_quoted & " " & ytdl_settings & " " & URL_user_entered & " " & YTDL_log_file & " " & download_filename & " " & download_filename_new & " " & MacYTDL_custom_icon_file_posix & " " & monitor_dialog_position & " " & YTDL_simulate_log & " " & diag_Title_quoted & " " & is_Livestream_Flag & " " & screen_width & " " & screen_height & " " & DL_Use_YTDLP & quoted form of path_to_MacYTDL
...
-- Initialise Help Menu - called in Main
-- Only loading Foundation when needed - simplifies changes to rest of code
--  
----------------------------------------------------------------------------------------------------------------------------
on initialise_help()
	script theScript
		...
		on initialise_help()
			try
				...
				set menuItem to appMenu's itemAtIndex:1
				menuItem's setAction:"displayHelp:"
				menuItem's setTarget:me
			on error errText
				display dialog "There was an error: " & errText
			end try
		end initialise_help
		on displayHelp:sender
			if current application's NSThread's isMainThread() then
				my performShowHelp:(missing value)
			else
				its performSelectorOnMainThread:"performShowHelp:" withObject:(missing value) waitUntilDone:true
			end if
		end displayHelp:
		on performShowHelp:sender
			display dialog "This will be the Help file if the code works"
		end performShowHelp:
	end script
	return theScript's initialise_help()
end initialise_help
