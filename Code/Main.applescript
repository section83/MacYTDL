...
-- symlinks are added to all script libaries by Shane Stanley's script copied from "Embedding script libraries that call other libraries" on LNSW forum
...
if YTDL_exists is false and ytdlp_exists is false and ffmpeg_exists is false and ffprobe_exists is false and prefs_exists is false and homebrew_ytdlp_exists is false and homebrew_ffmpeg_exists is false and homebrew_ffprobe_exists is false then
	...
	if components_install_answ is theButtonYesLabel then
		...
		set prefs_exists to true
		-- Commented out for v1.30, 5/11/25 as DTP is no longer installed in user's ~/Library/
		--		delay 1
		--		run_Utilities_handlers's install_DTP(DTP_file, path_to_MacYTDL, resourcesPath)
		--	set DTP_exists to true
		-- Commented out for v1.30, 5/11/25 as Myriad is no longer installed
		--	delay 1
...
if prefs_exists is false then
	...
	if Install_Prefs is theButtonYesLabel then
		-- v1.30, 29/11/25 - No longer need to check DTP as it's no longer installed
...
-- Added in v1.30, 1/11/25 - part of implementation of Deno, needed by yt-dlp - hopefully, getting Deno version on every startup is not too slow
...
on main_dialog(theButtonsHelpLabel, theButtonsUtilitiesLabel, theButtonsSettingsLabel, theButtonsAdminLabel, theFieldLabel, theCheckbox_Show_SettingsLabel, theCheckbox_SubTitlesLabel, theCheckbox_CredentialsLabel, theCheckbox_DescriptionLabel, thePathControlLabel, theCheckbox_OpenBatchLabel, theCheckbox_AddToBatchLabel, theDiagSettingsTextLabel)
	...
	if DL_QT_Compat is true then
		...
		-- v1.30, 2/12/25 - Changed from --recode-video to --exec to force FFmpeg to recode when container is already in mp4 format
	...
	-- v1.30, 25/11/25 changed YTDL_remux_format to YTDL_recode_remux - DL_Remux_format is not renamed but now controls both recode and remux
...
on utilities()
	...
	-- Added in v1.30, 1/11/25, part of iomplementation of Deno needed if yt-dlp is more recent than 2025.10.22
	if deno_version is "Not installed" then
		considering numeric strings
			if YTDL_version is greater than "2025.10.22" then
	...
	if utilities_button_number_returned is 5 then -- Start
		...
		try
			...
			if deno_version is not "Not installed" then
