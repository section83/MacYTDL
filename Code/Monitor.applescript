	...
	repeat
		...
		if YTDL_response is not "No response" then
			...
			if YTDL_response_lastParapraph contains "[download]" and YTDL_response_lastParapraph contains "%" then
				...
				set firstWordInPercentage to first word in progress_percentage -- <= CAN'T REMEMBER WHY THIS IS HERE
				...
				set GiBPositionAfterSize to offset of "GiB" in YTDL_response_lastParapraph
				if GiBPositionAfterSize is not 0 then
					...
					set approxIndicator to text 1 of downloadFileSizeGiB
		...
		if monitor_button_returned is "Stop" then
			...
			if download_finished is not "Yes" then
				...
				if download_filename_new_monitor_plain is "the multiple videos" or download_filename_new_monitor_plain is "the-playlist" then
					...
					set download_filename_new_monitor_trimmed to text 2 thru -5 of download_filename_new_monitor
					set download_filename_new_monitor_plain_trimmed to text 1 thru -5 of download_filename_new_monitor_plain
					set part_files to do shell script "find " & downloadsFolder_Path_monitor & " -maxdepth 1 -type f -iname *" & quoted form of download_filename_new_monitor_plain_trimmed & "*.part* -or -iname *" & quoted form of download_filename_new_monitor_plain_trimmed & "*.ytdl* -or  -iname *" & quoted form of download_filename_new_monitor_trimmed & "*.part* -or -iname *" & quoted form of download_filename_new_monitor_trimmed & "*.ytdl*"
					repeat with each_part_files in (get paragraphs of part_files)
						do shell script "mv " & quoted form of each_part_files & " ~/.trash/"
					end repeat
				end if
