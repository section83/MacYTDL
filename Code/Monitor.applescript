...
--  This script provides a download Monitor for each video file download requested by user - it is called by download_video() in main.scpt
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
on run {downloadsFolder_Path_monitor, MacYTDL_preferences_path_monitor, YTDL_TimeStamps_monitor, ytdl_settings_monitor, URL_user_entered_monitor, YTDL_log_file_monitor, download_filename_monitor, download_filename_new_monitor, MacYTDL_custom_icon_file_posix_monitor, monitor_dialog_position, YTDL_simulate_log_monitor, diag_Title_quoted_monitor, is_Livestream_Flag_monitor, screen_width, screen_height, DL_Use_YTDLP, path_to_MacYTDL, YTDL_Delete_Partial, ADL_Clear_Batch}
	...
	-- Turn on try/end try block to help diagnose problems	
	--	try
	...
	-- v1.31, 18/5/26 - Tryoing to find file name glitch
	--	display dialog "STOP HERE - download_filename_new_monitor: " & download_filename_new_monitor
	...
	set pathToBundleShort to text 1 thru -51 of pathToBundle
	...
	-- Get pid of this Monitor instance to send to Adviser - assume this is the most recent osascript process
	...
	if download_filename_new_monitor contains "#$" then -- This is a playlist to be downloaded in parallel
		...
		repeat with x from 1 to number_paragraphs
			...
			-- v1.31, 18/5/26 - Trying to find cause of parallel DL glitch - filename looks OK here
			--	display dialog "download_logfile_Parallel_not_quoted: " & download_logfile_Parallel_not_quoted
			
			-- v1.31, 18/5/26 - Trying to find filename glitch - must sent quoted log file name to shell
			--	set item x of ytdl_parallel_pid to do shell script shellPath & "cd " & downloadsFolder_Path_monitor_quoted & " ; " & YTDL_TimeStamps_monitor_quoted & " " & monitor_DL_Use_YTDLP & " " & ytdl_settings_monitor & " " & download_URL_parallel_quoted & " &> " & download_logfile_Parallel_not_quoted & " & echo $!"
		...
		-- Set up for parallel downloads of batches and multiple downloads	- beware, poor logic here as there are double hash delimiters in parallel playlist cases
		...
		set AppleScript's text item delimiters to {"##"} -- This is the delimiter added by main_dialog() to download_filename_new_monitor when user wants parallel downloads
		repeat with x from 1 to number_paragraphs
		...
		set path_to_scripts to text 1 thru -15 of path_to_monitor
		-- set myAdviserScriptAsString to quoted form of POSIX path of (path_to_scripts & "adviser.scpt")
		set myAdviserScriptAsString to quoted form of POSIX path of (path_to_scripts & "adviser.scptd")
		...
		set theConvertedInLabel to localized string "converted in" in bundle file pathToBundleShort from table "MacYTDL"
		...
		if download_filename_new_monitor_plain contains "\\'" then
			set download_filename_new_monitor_plain to replace_chars(download_filename_new_monitor_plain, "'\\'", "")
			set download_filename_new_monitor_plain to replace_chars(download_filename_new_monitor_plain, "\\''", "")
			set download_filename_new_monitor_plain to replace_chars(download_filename_new_monitor_plain, "\\'", "")
		end if
		...
		-- v1.29.3 - Tahoe mucks width of Monitor dialog - this tries to fix it - it's because macOS versions are now in a different format
		-- v1.30 - Looks like three part version number is a problem e.g. 26.0.1 - Nope not a problem here but, was so in DTP
		-- ********************************************************************************************************************************************************************************
		...
		considering numeric strings
			if user_os_version is greater than or equal to 26 then
				set accViewWidth to accViewWidth + 25
			end if
		end considering
		-- ********************************************************************************************************************************************************************************
		
		-- v1.31, 14/4/26 - Set up Monitor display counter - There is a limit on counter in effort to stop Monitor from hanging - Current limit is 300
		set monitor_display_count to 0
		
		
		-- It can take time for log file to get content - delay Monitor a bit
		delay 2
		
		-- v1.29 - Added this hoping to make Monitor frontmost - but found not useful so commented for now
		--			tell me to activate
		...
		repeat
			...
			if size_of_log is less than 5 then
				...
				try
					-- v1.30, 14/12/25 - Changed method for forming YTDL_log variable to reduce risk of it getting too big
					--	set YTDL_log to read file YTDL_log_file_monitor_posix as text
					set YTDL_log to last item of (read YTDL_log_file_monitor_posix using delimiter linefeed) as text
				on error errMsg
			...
			if YTDL_log is not "No log" then
				-- v1.30, 14/12/25 - No need to find last paragraph as it's set in the "read" call above - keeping YTDL_log variable rather than channging lots of code
				-- set YTDL_log_lastParapraph to paragraph -2 of YTDL_log
				set YTDL_log_lastParapraph to YTDL_log
				...
				if YTDL_log_lastParapraph contains "[download]" and YTDL_log_lastParapraph contains "%" and YTDL_log_lastParapraph contains "at" then
					...
					if YTDL_log contains "elapsed=" then
						set theDownConvertLabel to theConvertedInLabel
					else
						set theDownConvertLabel to theDownloadInLabel
					end if
					...
					set diag_intro_text_2 to theProgressLabel & " " & sizeOfdownloadProgressDelimited & "KB" & " " & theDownConvertLabel & " " & time_running & " " & theMinutesLabel
				end if
			end if
			
			-- Set variables for Monitor dialog which need to be updated with each repeat
			-- v1.29.3 - Changed minWidth to accViewWidth in hope of fixing Monitor in Tahoe without stuffing older OSs
			...
			set {intro_label2, theTop} to create label diag_intro_text_2 left inset 0 bottom 5 max width accViewWidth - 20 aligns center aligned control size small size with multiline
			...
			set {intro_label1, theTop} to create label diag_intro_text_1 left inset 50 bottom theTop + 10 max width accViewWidth - 51 control size small size
			...
			-- v1.31, 14/4/26 - Increment counter - There is a limit on counter in effort to stop Monitor from hanging - Current limit is 300
			set monitor_display_count to monitor_display_count + 1
			...
			if monitor_button_returned is theButtonsPauseLabel then
				...
				-- Try loop for testing the pause and resume function				
				--					try
				...
				if download_filename_new_monitor_noquotes_trimmed contains "\\'" then
					set download_filename_new_monitor_noquotes_trimmed to replace_chars(download_filename_new_monitor_noquotes_trimmed, "'\\'", "")
					set download_filename_new_monitor_noquotes_trimmed to replace_chars(download_filename_new_monitor_noquotes_trimmed, "\\''", "")
					set download_filename_new_monitor_noquotes_trimmed to replace_chars(download_filename_new_monitor_noquotes_trimmed, "\\'", "")
				end if
				set YTDL_log_file_resume to MacYTDL_preferences_path_monitor & "ytdl_log-" & download_filename_new_monitor_noquotes_trimmed & "-" & download_date_time & ".txt"
				set my_params to quoted form of downloadsFolder_Path_monitor & " " & quoted form of MacYTDL_preferences_path_monitor & " " & quoted form of YTDL_TimeStamps_monitor & " " & quoted form of ytdl_settings_monitor & " " & quoted form of URL_user_entered_monitor & " " & quoted form of YTDL_log_file_resume & " " & quoted form of download_filename_monitor & " " & quoted form of download_filename_new_monitor & " " & quoted form of MacYTDL_custom_icon_file_posix_monitor & " " & monitor_dialog_position & " " & quoted form of YTDL_simulate_log_monitor & " " & quoted form of diag_Title_quoted_monitor & " " & is_Livestream_Flag_monitor & " " & screen_width & " " & screen_height & " " & monitor_DL_Use_YTDLP & " " & quoted form of path_to_MacYTDL & " " & YTDL_Delete_Partial & " " & ADL_Clear_Batch
				...
				-- v1.31, 14/4/26 - Added test on monitor_display_count - count of 300 should be 25 minutes after which time Monitor should automatically die - TEST WITH 3 TO START
				-- Downloading, recoing and remuxing etc continue until finished - Adviser will then show which is nice.
				-- For testing which seems to work
				-- Count of 300 is 1,500 seconds or 25 minutes
				-- display dialog "Trigger count is: " & monitor_display_count
				--	else if monitor_button_returned is theButtonsStopLabel or monitor_display_count is 3 then
				-- Decided that it's better to keep downloading and conversion process going - during testing, found that it did complete even tho Monitor did a hang
				--				else if monitor_display_count is 3 then
			else if monitor_display_count is 300 then
				exit repeat
			...
			-- v1.30, 14/12/25 - Added this to be sure that content of YTDL_log and YTDL_log_lastParapraph are not retained across iterations of the Monitor main repeat loop - doesn't stop Monitor hanging
			set YTDL_log to ""
			set YTDL_log_lastParapraph to ""
...
on show_parallel_Monitor(download_logfile_Parallel_List, download_filename_Parallel_List, pathToBundleShort, MacYTDL_custom_icon_file_posix_monitor, X_position_monitor, Y_position_monitor, ytdl_parallel_pid, MacYTDL_preferences_path_monitor, batch_parallel_flag, downloadsFolder_Path_monitor, YTDL_Delete_Partial, ADL_Clear_Batch)
	...
	-- It can take time for log file to get content - delay Monitor a bit
	-- v1.29 - Decided to reduce delay from 2 to 1 second
	delay 1
	...
	-- No pause function as currently no way to indicate which download is to be paused
	repeat
		...
		if playlist_name is "" then
			...
			set diag_intro_text_1 to diag_intro_text_1 & " “" & playlist_name & "”"
		end if
		...
		if monitor_button_returned is theButtonsStopLabel then
			...
			repeat with x from 1 to number_of_downloads
				...
				if YTDL_Delete_Partial is "true" then
					...
					-- v1.31, 27/6/26 - Found that at least some iView file names did have underscores and so were not being deleted - decided to do two passes one with underscores and one without
					set download_filename_noreplace to download_filename
					set download_filename_noreplace_trimmed to text 1 thru -5 of download_filename_noreplace
					set part_files to do shell script "find " & downloadsFolder_Path_monitor & " -maxdepth 1 -type f -iname *" & (quoted form of download_filename_noreplace_trimmed) & "*part* -or -iname *" & (quoted form of download_filename_noreplace_trimmed) & "*ytdl* -or -iname *" & (quoted form of download_filename_noreplace_trimmed) & "*-Frag* -or -iname *" & (quoted form of download_filename_noreplace_trimmed) & "*f???*"
					repeat with each_part_files in (get paragraphs of part_files)
						do shell script "mv " & quoted form of each_part_files & " ~/.trash/"
					end repeat
					
					set download_filename to replace_chars(download_filename, "_", " ")
		...
		if process_finished_count is number_of_downloads then
			if monitor_button_returned is not theButtonsStopLabel then
				...
				show_Adviser(pathToBundleShort, theButtonsCloseLabel, downloadsFolder_Path_monitor, download_logfile_Parallel_List, MacYTDL_preferences_path_monitor, ADL_Clear_Batch, batch_parallel_flag)
			end if
			exit repeat
		end if
...
-- Since v1.27, had logic error which assumed downloads are always in a batch - Fixed
on show_Adviser(pathToBundleShort, theButtonsCloseLabel, downloadsFolder_Path_monitor, download_logfile_Parallel_List, MacYTDL_preferences_path_monitor, ADL_Clear_Batch, batch_parallel_flag)
	
	set alerterPath to quoted form of (POSIX path of (pathToBundleShort & "Contents:Resources:"))
	-- Try to reduce Adviser overlapping with Monitor – don't know why/how they can be displayed at same time !
	delay 1
	-- Check log files for errors
	set error_count to 0
	set number_of_downloads to number of items in download_logfile_Parallel_List
	if batch_parallel_flag is true then
	...
	--	set subtitleText to quoted form of ("“" & download_filename_new_monitor_plain & "”")    -- Might be able to provide playlist name or similar one day
	if error_count is greater than 0 then
		if ADL_Clear_Batch is "true" and batch_parallel_flag is true then
