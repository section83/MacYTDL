...
-- Variables for storing youtube-dl, FFmpeg, FFmprobe and DialogToolkitPlus locations
...
tell application "System Events"
	...
	if exists file ffprobe_file then
		...
		set ffmprobe_version_start to (offset of "version" in ffprobe_version_long) + 8
		set ffmprobe_version_end to (offset of "-tessus" in ffprobe_version_long) - 1
		set ffmprobe_version to text ffmprobe_version_start thru ffmprobe_version_end of ffprobe_version_long
	end if
end tell

-- Call handler to check whether FFmpeg (and FFprobe) is installed
check_ffmpeg_installed()


-- Check whether DTP exists; if not, call DTP installer
tell application "System Events"
...
on main_dialog()
	...
	-- Make sure user gets desired subtitle format by using --convert except where format is not valid for the convert parameter
	if subtitles_choice is true and DL_subtitles_format is not "best" and DL_subtitles_format is not "ttml" then
		set YTDL_subtitles to "--write-sub --convert-subs " & DL_subtitles_format & " "
	else if subtitles_choice is true and DL_subtitles_format is "best" then
		set YTDL_subtitles to "--write-sub --sub-format best "
	else if subtitles_choice is true and DL_subtitles_format is "ttml" then
		set YTDL_subtitles to "--write-sub --sub-format ttml "
	else
	...
	if DL_STEmbed is true then
		...
		--		set YTDL_subtitles to "--write-sub "     -- <= remove as need to enable subtitles conversion as well as embedding
	else
		set YTDL_STEmbed to ""
	end if
	...
	else if button_returned is "Continue" then
		if openBatch_chosen is true then
			open_batch_processing(folder_chosen, remux_format_choice, YTDL_credentials, YTDL_subtitles, YTDL_STEmbed, YTDL_format, YTDL_remux_format, YTDL_Remux_original, YTDL_description, YTDL_audio_only, YTDL_over_writes, YTDL_Thumbnail_Write, YTDL_Thumbnail_Embed, YTDL_metadata, YTDL_verbose)
		else
			download_video(folder_chosen, remux_format_choice, YTDL_credentials, YTDL_subtitles, YTDL_STEmbed, YTDL_format, YTDL_remux_format, YTDL_Remux_original, YTDL_description, YTDL_audio_only, YTDL_over_writes, YTDL_Thumbnail_Write, YTDL_Thumbnail_Embed, YTDL_metadata, YTDL_verbose)
		end if
...
on download_video(folder_chosen, remux_format_choice, YTDL_credentials, YTDL_subtitles, YTDL_STEmbed, YTDL_format, YTDL_remux_format, YTDL_Remux_original, YTDL_description, YTDL_audio_only, YTDL_over_writes, YTDL_Thumbnail_Write, YTDL_Thumbnail_Embed, YTDL_metadata, YTDL_verbose)
	...
	if YTDL_simulate_response contains "WARNING" then
		if YTDL_simulate_response does not contain "Falling back on generic information" then
			...
			if warning_quit_or_continue is "Check for Updates" then
				check_ytdl()
				check_ffmpeg()
				display dialog alert_text_ytdl & return & return & alert_text_ffmpeg with title diag_Title buttons {"OK"} default button {"OK"} with icon note giving up after 600
				main_dialog()
...
on set_File_Names(YTDL_simulate_response)
	...
	if show_name is "" then -- not an iView show page
		...
		if myNum is 0 then
			...
			set YTDL_output_template to " -o '%(title)s.%(ext)s'"
			set download_filename_new to text 1 thru -5 of (do shell script shellPath & "cd " & downloadsFolder_Path & " ; " & "youtube-dl --get-filename --restrict-filenames " & URL_user_entered & " " & YTDL_output_template)
			set YTDL_response_file to MacYTDL_preferences_path & "youtube-dl_response-" & download_filename_new & "-" & download_date_time & ".txt"
			...
			set download_filename to text 1 thru -5 of (do shell script shellPath & "cd " & downloadsFolder_Path & " ; " & "youtube-dl --get-filename --restrict-filenames " & URL_user_entered & " " & YTDL_output_template_get_name)
...
on set_settings(URL_user_entered_clean)
	...
	set {settings_thePopUp_SubTitlesFormat, settings_STFormatlabel, theTop} to create labeled popup {"best", "srt", "vtt", "ass", "ttml"} left inset 245 bottom (theTop) popup width 100 max width 200 label text "Subtitles format:" popup left 350 initial choice DL_subtitles_format
	...
	set settings_allControls to {theSettingsRule, settings_theCheckbox_Show_Settings, settings_theCheckbox_Original, settings_thePopUp_RemuxFormat, settings_remuxlabel, settings_theCheckbox_Metadata, settings_theCheckbox_Verbose, settings_theCheckbox_ThumbEmbed, settings_theCheckbox_ThumbWrite, settings_thePopUp_SubTitlesFormat, settings_STFormatlabel, settings_theCheckbox_STEmbed, settings_theCheckbox_SubTitles, settings_theCheckbox_Auto_YTDL_Check, settings_theCheckbox_OverWrites, settings_theCheckbox_AudioOnly, settings_theCheckbox_Description, settings_thePopUp_FileFormat, settings_formatlabel, settings_thePathControl, settings_pathLabel, MacYTDL_icon, settings_prompt}
	...
	if settings_button_returned is "Save Settings" then
		...
		set settings_show_settings_choice to item 2 of settings_controls_results -- <= Keep original after remux choice
		...
		set settings_stembed_choice to item 12 of settings_controls_results -- <= Embed subtitles choice
		set settings_subtitles_choice to item 13 of settings_controls_results -- <= Subtitles choice
		set settings_YTDL_auto_choice to item 14 of settings_controls_results -- <= Auto check YTDL version on startup choice
		set settings_over_writes_choice to item 15 of settings_controls_results -- <= Over-writes choice
		set settings_audio_only_choice to item 16 of settings_controls_results -- <= Audio only choice
		set settings_description_choice to item 17 of settings_controls_results -- <= Description choice
		set settings_format_choice to item 18 of settings_controls_results -- <= File format choice
		set settings_choice_19 to item 19 of settings_controls_results -- <= The Format popup label
		set settings_folder_choice to item 20 of settings_controls_results -- <= The download path choice
		set settings_choice_21 to item 21 of settings_controls_results -- <= The Path label
		set settings_choice_22 to item 21 of settings_controls_results -- <= The MacYTDL icon
		--		set settings_choice_23 to item 23 of settings_controls_results -- <= Contains the "About" text
		...
		if settings_subtitles_choice is true and settings_subtitlesformat_choice is not "best" and settings_subtitlesformat_choice is not "ttml" then
...
on check_ytdl()
	...
	if YTDL_page is "" then
		...
		-- Starting postion should always be 31 but best not to assume.  Will, however, assume version is always surrounded by "(v" and "("
		-- Need to scan YTDL_page for "(v" to get starting position then scan for ")" to get ending position - the data between those is the version number
...
-- Handler for installing FFmpeg and FFprobe - called by main thread
on check_ffmpeg_installed()
	if ffmpeg_version is "Not installed" then
		...
		if FFmpeg_page is "" then
			...
			set ffmpeg_install_text to "FFmpeg is not installed." & return & return & "Would you like to install it now ?  If yes, this will also install FFprobe.  If not, MacYTDL can't run and you should quit.  Note: You may need to provide administrator credentials."
			tell me to activate
	...
	if FFmpeg_page is "" then
		...
		if ffmpeg_version_check is not equal to ffmpeg_version then
			...
			if ffmpeg_install_answ is "Yes" then
				...
				set copy_to_path to "/usr/local/bin/"
				try
					do shell script "unzip -o " & downloadsFolder_Path & "/ffmpeg-4.2.zip -d " & copy_to_path with administrator privileges
					do shell script "chmod a+x /usr/local/bin/ffmpeg" with administrator privileges
					do shell script "rm " & ffmpeg_download_file
					...
					do shell script "unzip -o " & downloadsFolder_Path & "/ffprobe-4.2.zip -d " & copy_to_path with administrator privileges
					do shell script "chmod a+x /usr/local/bin/ffprobe" with administrator privileges
				on error number -128
					-- trap case where user cancels credentials dialog
					main_dialog()
...
on utilities()
	...
	set {utilities_service_status, theTop} to create label service_installed left inset 275 bottom (theTop - 155) max width 225 aligns left
	set {utilities_atomic_status, theTop} to create label Atomic_installed left inset 275 bottom (theTop + 5) max width 225 aligns left
	...
	if utilities_button_returned is "Start" then
		...
		if utilities_Atomic_choice is true then
			if Atomic_installed is "Atomic Parsley installed          No" then
		...
		if utilities_Service_choice is true then
			if service_installed is "MacYTDL Service installed     No" then
...
on update_MacYTDLservice()
	...
	if Service_exists_flag is "Yes" then
		...
		if old_service_size is not equal to new_service_size then
			do shell script "mv " & quoted form of (old_service_file) & " ~/.trash/Send-URL-To-MacYTDL.workflow"
...
on Get_ABC_Episodes(URL_user_entered)
	...
	if myNum is greater than 0 then
		...
		if ABC_button_returned is "Download" then
			-- Get checkbox results from ABC show dialog - process in reverse order - result will become "URL_user_entered" back in Main()
