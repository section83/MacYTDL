...
-- use script "DialogToolkitMacYTDL" -- Yosemite (10.10) or later - 1.30, 5/12/25 - Currently not working when this script called from Automator

-- Define variables to be filled by the read_settings() handler below - makes these variables available to rest of auto_download
...
--global myNum -- v1.30, 5/11/25 - Not used as no longer using iView and SBS choosers in auto_download
...
on auto_Download(MacYTDL_prefs_file, URL_user_entered_clean, path_to_MacYTDL)
	...
	-- v1.30, 2/11/25 - Added deno_version in less complicated way of getting deno_version
	-- 29/11/25 - Copied code from Preliminaries in order to properly set deno_version
	-- v1.30, 16/12/25 - Realised that Deno version had to be stored in user's plist - Bum - Anyway, makes startups quicker
	...
	-- v1.30,21/12/25 - comment out as not relevant to new Service - but keep user_on_old_os to maintain some consistency with main.scpt
	...
	-- v1.31, 2/5/26 - Added merge-output-format
	...
	-- Questions: subtitles_choice [where is it defined ? Have reverted to DL_subtitles here but why use it in download_video() ?]; URL_user_entered_from_auto_download [what is purpose in download_video() ? Have reverted to URL_user_entered_clean]
...
----------------------------------------------------------------------------------------------------
--
-- 	Check cookies - called by auto_Download - if user turns on use_cookies
--
----------------------------------------------------------------------------------------------------
...
----------------------------------------------------------------------------------------------------
--
-- 	Check downloads folder - called by auto_Download
--
----------------------------------------------------------------------------------------------------
...
on download_video(shellPath, path_to_MacYTDL, MacYTDL_custom_icon_file, MacYTDL_custom_icon_file_posix, screen_width, screen_height, YTDL_simulate_file, URL_user_entered, URL_user_entered_from_auto_download, folder_chosen, diag_Title, DL_batch_status, DL_Remux_format, subtitles_choice, YTDL_videoPW, YTDL_credentials, YTDL_subtitles, YTDL_STEmbed, YTDL_format_pref, YTDL_format, YTDL_recode_remux, YTDL_Merge_Format, YTDL_Remux_original, YTDL_description, YTDL_audio_only, YTDL_audio_codec, YTDL_over_writes, YTDL_Thumbnail_Write, YTDL_Thumbnail_Embed, YTDL_metadata, YTDL_limit_rate_value, YTDL_verbose, YTDL_TimeStamps, YTDL_Use_Proxy, YTDL_Get_Cookies, YTDL_Use_Cookies, YTDL_Custom_Settings, YTDL_Custom_Template, YTDL_no_part, skip_Main_dialog, theButtonOKLabel, theButtonCancelLabel, theButtonDownloadLabel, theButtonReturnLabel, theButtonQuitLabel, theButtonContinueLabel, YTDL_QT_Compat, DL_Use_YTDLP, theBestLabel, YTDL_Resolution_Limit, YTDL_Use_Parts, YTDL_No_Warnings, ADL_Clear_Batch, user_on_old_os, X_position, Deno_version, YTDL_Use_netrc, YTDL_Use_netrc_Location, YTDL_version)
			...
			considering numeric strings
				if YTDL_version is greater than "2025.10.22" then
					...
					if install_deno_query is theButtonInstall then
						...
						set Deno_version to my install_update_Deno(Deno_version, user_system_arch, resourcesPath, path_to_MacYTDL, theButtonOKLabel, MacYTDL_custom_icon_file, diag_Title, MacYTDL_prefs_file)
						if Deno_version is "Main" then return
	...
	-- Replace spaces, semi-colons and combinations of those with commas to indicate individual items
	...
	if URL_user_entered contains "https://iview.abc.net.au/show/" then
		...
		if iView_show_type is not "Movie" and iView_show_type is not "TVEpisode" then
			...
			set theiViewAskForEpisodesLabel2 to localized string "To choose episodes, type the episode number(s) in the box below (commas to separate episode numbers and colons for a range). Episodes are in the order shown on the iView page. For all episodes leave the box blank." from table "MacYTDL"
	...
	-- v1.31 - 6/4/26 - No need for Alert on playlist simulations which are now much faster
	-- v1.31 - 6/4/26 - Provide too many playlist items check for parallel downloads - can't remember why it was omitted.=
	...
	if URL_user_entered_clean contains "&index=" and URL_user_entered_clean contains "youtu" then
		...
		-- v1.31, 27/6/26 - Apply soft limit on number of playlist items
	...
	-- v1.31 - 6/4/26 - Separate simulate for playlists - enables much faster process
	...
	set YTDL_simulate_log to my replace_chars(YTDL_simulate_log, {"True#", "False#", "NA#"}, "")
	...
	-- *****************************************************************************
	-- Setting ABC and SBS show page variables here for now
	-- *****************************************************************************	
	...
	-- Ninth, look for any more warnings in simulate file. Get filename from the simulate log file
	...
	--	set ytdl_settings to quoted form of (" --ignore-errors --newline " & YTDL_subtitles & YTDL_STEmbed & YTDL_videoPW & YTDL_credentials & YTDL_format & YTDL_Resolution_Limit & YTDL_Use_Parts & YTDL_No_Warnings & YTDL_recode_remux & YTDL_Remux_original & YTDL_description & YTDL_audio_only & YTDL_audio_codec & YTDL_over_writes & YTDL_Thumbnail_Write & YTDL_Thumbnail_Embed & YTDL_metadata & YTDL_limit_rate_value & YTDL_verbose & YTDL_Use_Proxy & YTDL_Use_Cookies & YTDL_no_playlist & YTDL_no_part & YTDL_Custom_Settings & YTDL_output_template & " " & YTDL_QT_Compat & " " & YTDL_formats_to_download & " " & YTDL_Use_netrc & YTDL_Use_netrc_Location & " ")
	set ytdl_settings to quoted form of (" --ignore-errors --newline " & YTDL_subtitles & YTDL_STEmbed & YTDL_videoPW & YTDL_credentials & YTDL_format & YTDL_Resolution_Limit & YTDL_Use_Parts & YTDL_No_Warnings & YTDL_recode_remux & YTDL_Merge_Format & YTDL_Remux_original & YTDL_description & YTDL_audio_only & YTDL_audio_codec & YTDL_over_writes & YTDL_Thumbnail_Write & YTDL_Thumbnail_Embed & YTDL_metadata & YTDL_limit_rate_value & YTDL_verbose & YTDL_Use_Proxy & YTDL_Get_Cookies & YTDL_Use_Cookies & YTDL_no_playlist & YTDL_no_part & YTDL_Custom_Settings & YTDL_output_template & " " & YTDL_QT_Compat & " " & YTDL_Use_netrc & YTDL_Use_netrc_Location & " ")
				...
				if length_each_filename is not 0 then
					...
					if search_for_download is not {} then
						...
						if overwrite_continue_choice is theShowExistsButtonOverwriteLabel then
							...
							set file_to_delete to quoted form of (POSIX path of (downloadsFolder_Path & "/" & search_for_download))
							do shell script "mv " & file_to_delete & " ~/.trash/"
							set ytdl_settings to quoted form of (" --ignore-errors --newline " & YTDL_subtitles & YTDL_STEmbed & YTDL_videoPW & YTDL_credentials & YTDL_format & YTDL_Resolution_Limit & YTDL_Use_Parts & YTDL_No_Warnings & YTDL_recode_remux & YTDL_Merge_Format & YTDL_Remux_original & YTDL_description & YTDL_audio_only & YTDL_audio_codec & YTDL_over_writes & YTDL_Thumbnail_Write & YTDL_Thumbnail_Embed & YTDL_metadata & YTDL_limit_rate_value & YTDL_verbose & YTDL_Use_Proxy & YTDL_Get_Cookies & YTDL_Use_Cookies & YTDL_no_playlist & YTDL_no_part & YTDL_Custom_Settings & YTDL_output_template & " " & YTDL_QT_Compat & " " & YTDL_Use_netrc & YTDL_Use_netrc_Location & YTDL_verbose & YTDL_Use_Proxy & YTDL_Get_Cookies & YTDL_Use_Cookies & YTDL_no_playlist & YTDL_no_part & YTDL_Custom_Settings & YTDL_output_template & " " & YTDL_QT_Compat & " " & YTDL_Use_netrc & YTDL_Use_netrc_Location & " ")
...
-- deno_version contains current version installed, "Not installed"
...
-- v1.28 - Took quite a lot of testing to get this working correctly - don't know what the purpose of the reversed variable was
...
-- v1.31, 27/6/26 - Decided to omit the quit funciton for auto_download - don't think it does anything useful
---------------------------------------------------
--
