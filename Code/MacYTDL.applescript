...
on download_video(folder_chosen, remux_format_choice, subtitles_choice, YTDL_credentials, YTDL_subtitles, YTDL_STEmbed, YTDL_format, YTDL_remux_format, YTDL_Remux_original, YTDL_description, YTDL_audio_only, YTDL_over_writes, YTDL_Thumbnail_Write, YTDL_Thumbnail_Embed, YTDL_metadata, YTDL_verbose)
	...
	-- Initialise indicator which will show whether URL is for an ABC show page - needed for over-writing code below
	set ABC_show_indicator to "No"
	
	-- Do a simulation to get back file name and disclose any errors or warnings
	-- Accessed iView show page causes error => takes processing to Get_ABC_Episodes handler
	...
	try
		do shell script shellPath & "cd " & quoted form of downloadsFolder_Path & " ; " & "youtube-dl --get-filename " & YTDL_format_pref & YTDL_credentials & URL_user_entered_clean_quoted & " " & YTDL_output_template & " > /dev/null" & " &> " & YTDL_simulate_file
	on error
		...
		else if URL_error contains "requested format not available" then
			set quit_or_return to button returned of (display dialog "Your preferred file format is not available.  Would you like to cancel download and return, have your download remuxed into your preferred format or just download the best format available ?" & return & "{Note: 3gp format is not available - a request for 3gp will be remuxed into mp4.}" buttons {"Return", "Remux", "Download"} default button "Download" with title diag_Title with icon note giving up after 600)
	...
	set ytdl_settings to quoted form of (" --ignore-errors --newline " & YTDL_subtitles & YTDL_STEmbed & YTDL_credentials & YTDL_format & YTDL_remux_format & YTDL_Remux_original & YTDL_description & YTDL_audio_only & YTDL_over_writes & YTDL_Thumbnail_Write & YTDL_Thumbnail_Embed & YTDL_metadata & YTDL_verbose & YTDL_output_template & " ")
	...
	if YTDL_over_writes is "--no-overwrites " then
		...
		-- But, first check whether it's an ABC show page - because the simulate result for those comes from the set_File_Names handler
		...
		if ABC_show_indicator is "Yes" then
			...
			repeat with each_filename in (get paragraphs of download_filename_new_plain)
				...
				if length_each_filename is not 0 then
					...
					if search_for_download is not {} then
						set overwrite_continue_choice to button returned of (display dialog "A file for the ABC show \"" & each_filename & "\" already exists." & return & return & "Do you want to continue anyway, download with a different name or stop and return to the main dialog ?" buttons {"Overwrite", "New name", "Cancel download"} default button "Cancel download" with title diag_Title with icon note giving up after 600)
						if overwrite_continue_choice is "Overwrite" then
							...
							set ytdl_settings to quoted form of (" --ignore-errors --newline " & YTDL_subtitles & YTDL_STEmbed & YTDL_credentials & YTDL_format & YTDL_remux_format & YTDL_Remux_original & YTDL_description & YTDL_audio_only & YTDL_metadata & YTDL_verbose & YTDL_output_template & " ")
							...
							set ytdl_settings to quoted form of (" --ignore-errors --newline " & YTDL_subtitles & YTDL_STEmbed & YTDL_credentials & YTDL_format & YTDL_remux_format & YTDL_Remux_original & YTDL_description & YTDL_audio_only & YTDL_over_writes & YTDL_metadata & YTDL_verbose & YTDL_output_template_new & " ")
			...
			if (count of paragraphs of download_filename_new_plain) is greater than 1 then
				set download_filename_new to show_name
			end if
		else
...
on set_File_Names(YTDL_simulate_response)
	...
	if show_name is "" then -- not an iView show page
		if number_of_URLs is 1 then -- Single file download or playlist
			...
			else if YTDL_simulate_response contains "WARNING:" and YTDL_simulate_response does not contain "ERROR:" then --<= Single file download but simulate.txt contains WARNING in first paragraph (iView and non-iView) - need to trim ".mp4<para>" from end of 2nd paragraph which is last line of file - but this fails when there are two or more paragraphs of warnings
				set download_filename to text 1 thru -1 of paragraph 2 of YTDL_simulate_response
				set download_filename_trimmed to text 1 thru -6 of paragraph 2 of YTDL_simulate_response
				set download_filename_new to replace_chars(download_filename, " ", "_")
				set download_filename_trimmed to replace_chars(download_filename_trimmed, " ", "_")
				set YTDL_response_file to MacYTDL_preferences_path & "youtube-dl_response-" & download_filename_trimmed & "-" & download_date_time & ".txt"
			else if YTDL_simulate_response contains "ERROR:" then --<= Single file download but simulate.txt contains ERROR in first paragraph (iView and non-iView) - need a generic file name
		...
		if myNum is 0 then
			...
			set download_filename_new to show_name & "-" & download_filename
			set download_filename_new to replace_chars(download_filename_new, " ", "_")
			set download_filename_trimmed to text 1 thru -5 of download_filename_new
			set YTDL_response_file to MacYTDL_preferences_path & "youtube-dl_response-" & download_filename_trimmed & "-" & download_date_time & ".txt"
		else
			...
			set download_filename_new to replace_chars(download_filename, " ", "_")
			set show_name_underscore to replace_chars(show_name, " ", "_")
			set YTDL_response_file to MacYTDL_preferences_path & "youtube-dl_response-" & show_name_underscore & "-" & download_date_time & ".txt"
		end if
	end if
...
on check_subtitles_download_available()
	...
	if check_subtitles_available does not contain "Language formats" then
		...
		repeat with response_subtitle_paragraph in response_ST_paragraphs
			-- Might need a more descriminating if test below of YTDL response can have other than 12 character paragraphs
...
on set_settings(URL_user_entered_clean)
	...
	if settings_button_returned is "Save Settings" then
		...
		set settings_audio_only_choice to item 17 of settings_controls_results -- <= Audio only choice
		set settings_description_choice to item 18 of settings_controls_results -- <= Description choice
		set settings_format_choice to item 19 of settings_controls_results -- <= File format choice
		set settings_choice_20 to item 20 of settings_controls_results -- <= The Format popup label
		set settings_folder_choice to item 21 of settings_controls_results -- <= The download path choice
		set settings_choice_22 to item 22 of settings_controls_results -- <= The Path label
		set settings_choice_23 to item 23 of settings_controls_results -- <= The MacYTDL icon
...
-- Handler to check if youtube-dl is installed and which version - install if user agrees but can't run MacYTDL without it - called by main thread
on check_ytdl_installed()
	if YTDL_version is "Not installed" then
		...
		if yt_install_answ is "Yes" then
			...
			set myScriptAsString to "display notification \"Download and install of youtube-dl started.  Please wait, ... .\" with title \"MacYTDL\""
...
on check_ytdl()
	-- Check version installed - done again because it might have been manually changed while MacYTDL was open
	tell application "System Events"
		if exists file youtubedl_file then
			tell current application
				set YTDL_version to do shell script youtubedl_file & " --version"
	...
	if YTDL_page is "" then
		...
		-- Trap download errors reported by curl
		if length of YTDL_version_check is greater than 15 then
			display dialog "There was a problem with downloading youtube-dl. There could be a problem with the server. Wait a short while and try again." buttons {"OK"} default button "OK" with title diag_Title with icon note giving up after 600
			main_dialog()
		end if
...
on check_MacYTDL()
	...
	if MacYTDL_releases_page is "" then
		...
		display dialog "MacYTDL_version_check: " & MacYTDL_version_check & return & "MacYTDL_version: " & MacYTDL_version
...
on check_ffmpeg_installed()
	...
	if FFmpeg_page is "" then
		...
		if ffprobe_version is "Not installed" and ffmpeg_version is "Not installed" then
			set ffmpeg_install_text to "FFmpeg and FFprobe are not installed." & return & return & "Would you like to install them now ?  You can start MacYTDL and change settings but, downloads will not work.  Note: You may need to provide administrator credentials."
		else if ffmpeg_version is "Not installed" then
			set ffmpeg_install_text to "FFmpeg is not installed." & return & return & "Would you like to install it now ?  You can start MacYTDL and change settings but, downloads will not work.  Note: You may need to provide administrator credentials."
		else if ffprobe_version is "Not installed" then
			set ffmpeg_install_text to "FFprobe is not installed." & return & return & "Would you like to install it now ? You can start MacYTDL and change settings but, downloads will not work.  Note: You may need to provide administrator credentials."
		end if
		tell me to activate
		set ffmpeg_install_query to display dialog ffmpeg_install_text buttons {"Quit", "Return", "Yes"} default button "Yes" cancel button "Quit" with title diag_Title with icon note giving up after 600
		...
		if ffmpeg_install_answ is "Return" then
			main_dialog()
		end if
		if ffmpeg_install_answ is "Yes" then
			if ffmpeg_version is "Not installed" then
				...
				set ffmpeg_download_file to quoted form of (copy_to_path & "/ffmpeg-" & ffmpeg_version_new & ".zip")
				try
					-- Download latest FFmpeg zip file to usr/local/bin, unzip, rm zip file
				...
				on error errStr number errorNumber
					if errorNumber is -128 then
						...
						display dialog "There was a problem with installing FFmpeg. This was the error message: " & errorNumber & " " & errStr & return & return & "You can run MacYTDL and change settings but downloads will not work until FFmpeg is installed. When you next start MacYTDL, it will try again to install FFmpeg." buttons {"OK"} default button "OK" with title diag_Title with icon note giving up after 600
					end if
				end try
			end if
			if ffprobe_version is "Not installed" then
				...
				set ffprobe_download_file to quoted form of (copy_to_path & "/ffprobe-" & ffprobe_version_new & ".zip")
				...
				on error errStr number errorNumber
					if errorNumber is -128 then
						...
						display dialog "There was a problem with installing FFprobe. This was the error message: " & errorNumber & " " & errStr & return & return & "You can run MacYTDL and change settings but downloads will not work until FFprobe is installed. When you next start MacYTDL, it will try again to install FFmpeg." buttons {"OK"} default button "OK" with title diag_Title with icon note giving up after 600
					end if
				end try
			end if
		end if
...
on check_ffmpeg()
	...
	if FFmpeg_page is "" then
		...
		if ffmpeg_version_check is not equal to ffmpeg_version then
			set ffmpeg_install_text to "FFmpeg is out of date. You have version " & ffmpeg_version & ".  The current version is " & ffmpeg_version_check & return & return & "Would you like to update it now ?  If yes, this will also update FFprobe.  Note: You may need to provide administrator credentials."
			tell me to activate
			...
			if ffmpeg_install_answ is "Yes" then
				...
				do shell script "osascript -e " & quoted form of myScriptAsString & " > /dev/null 2> /dev/null & "
				delay 1
				set ffmpeg_download_file to quoted form of (copy_to_path & "/ffmpeg-" & ffmpeg_version_check & ".zip")
...
on utilities()
	...
	if utilities_button_returned is "Start" then
		...
		if utilities_YTDL_check_choice is true and utilities_FFmpeg_check_choice is true then
			...
			if ffmpeg_version is "Not installed" then
				check_ffmpeg_installed()
				set alert_text_ffmpeg to "FFmpeg and FFprobe have been installed."
			else
				check_ffmpeg()
			end if
			tell me to activate
...
----------------------------------------------------------------------------------------------------------
--
-- 	Handlers to update format of Preferences file for v1.10, 1.11 and 1.12.1
--
----------------------------------------------------------------------------------------------------------
...
on open_batch_processing(folder_chosen, remux_format_choice, subtitles_choice, YTDL_credentials, YTDL_subtitles, YTDL_STEmbed, YTDL_format, YTDL_remux_format, YTDL_Remux_original, YTDL_description, YTDL_audio_only, YTDL_over_writes, YTDL_Thumbnail_Write, YTDL_Thumbnail_Embed, YTDL_metadata, YTDL_verbose)
	...
	if batch_button_returned is "Download" then
		...
		clear_batch(folder_chosen, remux_format_choice, subtitles_choice, YTDL_credentials, YTDL_subtitles, YTDL_STEmbed, YTDL_format, YTDL_remux_format, YTDL_Remux_original, YTDL_description, YTDL_audio_only, YTDL_over_writes, YTDL_Thumbnail_Write, YTDL_Thumbnail_Embed, YTDL_metadata, YTDL_verbose)
	else if batch_button_returned is "Remove last item" then
		remove_last_from_batch(folder_chosen, remux_format_choice, subtitles_choice, YTDL_credentials, YTDL_subtitles, YTDL_STEmbed, YTDL_format, YTDL_remux_format, YTDL_Remux_original, YTDL_description, YTDL_audio_only, YTDL_over_writes, YTDL_Thumbnail_Write, YTDL_Thumbnail_Embed, YTDL_metadata, YTDL_verbose)
	end if
...
on download_batch(folder_chosen, remux_format_choice, YTDL_subtitles, YTDL_STEmbed, YTDL_credentials, YTDL_format, YTDL_remux_format, YTDL_Remux_original, YTDL_description, YTDL_audio_only, YTDL_over_writes, YTDL_Thumbnail_Write, YTDL_Thumbnail_Embed, YTDL_metadata, YTDL_verbose)
	...
	set ytdl_settings to quoted form of (" --ignore-errors --newline " & YTDL_subtitles & YTDL_STEmbed & YTDL_credentials & YTDL_format & YTDL_remux_format & YTDL_Remux_original & YTDL_description & YTDL_audio_only & YTDL_over_writes & YTDL_Thumbnail_Write & YTDL_Thumbnail_Embed & YTDL_metadata & YTDL_verbose & YTDL_output_template & " --batch-file " & YTDL_batch_file)
...
on clear_batch(folder_chosen, remux_format_choice, subtitles_choice, YTDL_credentials, YTDL_subtitles, YTDL_STEmbed, YTDL_format, YTDL_remux_format, YTDL_Remux_original, YTDL_description, YTDL_audio_only, YTDL_over_writes, YTDL_Thumbnail_Write, YTDL_Thumbnail_Embed, YTDL_metadata, YTDL_verbose)
	-- Check that there is a batch file
	tell application "System Events"
		set batch_file_test to batch_file as string
		if not (exists file batch_file_test) then
...
on remove_last_from_batch(folder_chosen, remux_format_choice, subtitles_choice, YTDL_credentials, YTDL_subtitles, YTDL_STEmbed, YTDL_format, YTDL_remux_format, YTDL_Remux_original, YTDL_description, YTDL_audio_only, YTDL_over_writes, YTDL_Thumbnail_Write, YTDL_Thumbnail_Embed, YTDL_metadata, YTDL_verbose)
	-- Check that there is a batch file
	tell application "System Events"
		set batch_file_test to batch_file as string
		if not (exists file batch_file_test) then
