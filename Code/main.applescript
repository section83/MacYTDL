-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--  MacYTDL
--  A GUI for the Python script yt-dlp.  Many thanks to Shane Stanley, Mark Alldritt and all the kind people at forum.latenightsw.com and www.macscripter.net
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
global DL_Set_Merge_Format
global DL_Merge_Format
...
global DL_Get_Cookies
global DL_Cookies_Source
global DL_Use_Cookies
global DL_Cookies_Location
global DL_PL_Items_Limit
global DL_Use_Proxy
global DL_Proxy_URL
global DL_Use_netrc
global DL_Netrc_Location
global DL_Use_Netrc_Location
...
if path_to_MacYTDL_posix contains "/Volumes/MacYTDL" or path_to_MacYTDL_posix contains "AppTranslocation" then
...
tell application "System Events"
	...
	if exists file ytdlp_file then
		...
		-- v1.31, 2/5/26 - put size of ytdl install into variable so it's not calculated twice
		set size_ytdl_install to size of ytdlp_path_alias
		--		if size of ytdlp_path_alias is greater than 9000 then
		if size_ytdl_install is greater than 9000 then
		...
		if size_ytdl_install is less than 9000 and its_type is not "slnk" then
			set homebrew_ytdlp_exists to false
			set ytdlp_exists to false
		end if
...
if DL_YTDL_auto_check is true then
	...
	if length of YTDL_version is greater than 12 then
		set alert_text_ytdlLabel to localized string "Do you wish to install the current nightly build of yt-dlp" from table "MacYTDL"
		...
		if update_nightly_q is theButtonOKLabel then
			...
			try
				...
				if show_yt_dlp is "yt-dlp" then
					...
					-- v1.31, 12/4/26 - Why is this here ? Users on final legacy can't be on any nor update to any nightly
					...
					--		do shell script "curl -L -k https://github.com/yt-dlp/yt-dlp/releases/download/2025.08.11/yt-dlp_macos_legacy  -o /usr/local/bin/yt-dlp" with administrator privileges
			...
			if cancel_update_flag is false then
				-- v1.30, 14/11/25 - Now getting yt-dlp version from release web page and updating YTDL_YTDLP_version - seems faster
		...
		-- User using yt-dlp stable release
...
set theCheckbox_VideoPasswordLabel to localized string "Video password" from table "MacYTDL"
...
on main_dialog(theButtonsHelpLabel, theButtonsUtilitiesLabel, theButtonsSettingsLabel, theButtonsAdminLabel, theFieldLabel, theCheckbox_Show_SettingsLabel, theCheckbox_SubTitlesLabel, theCheckbox_CredentialsLabel, theCheckbox_VideoPasswordLabel, theCheckbox_DescriptionLabel, thePathControlLabel, theCheckbox_OpenBatchLabel, theCheckbox_AddToBatchLabel, theDiagSettingsTextLabel)
	...
	set {theCheckbox_VideoPW, theTop} to create checkbox theCheckbox_VideoPasswordLabel left inset accViewInset bottom (theTop + 5) max width 200 without initial state
	...
	set {button_label_returned, button_number_returned, controls_results, finalPosition} to display enhanced window diag_Title acc view width (accViewWidth + add_extra_dialog_width) acc view height theTop acc view controls {theField, theCheckbox_Show_Settings, theCheckbox_SubTitles, theCheckbox_VideoPW, theCheckbox_Credentials, theCheckbox_Description, thePathControl, theCheckbox_AddToBatch, theCheckbox_OpenBatch, pathLabel, diag_settings_prompt, theRule, MacYTDL_icon} buttons theButtons active field theField initial position window_Position
	...
	set videoPW_choice to item 4 of controls_results
	set credentials_choice to item 5 of controls_results
	set description_choice to item 6 of controls_results
	--	set remux_format_choice to item 7 of controls_results   --   No longer have remux as runtime option
	set folder_chosen to item 7 of controls_results
	set DL_batch_status to item 8 of controls_results
	set openBatch_chosen to item 9 of controls_results
	...
	-- v1.31, 1/5/26 - Add merge output format - an alternative to remux
	...
	if DL_Set_Merge_Format is true then
		set YTDL_Merge_Format to " --merge-output-format " & DL_Merge_Format & " "
	else
		set YTDL_Merge_Format to ""
	end if
	...
	set YTDL_videoPW to ""
	if videoPW_choice is true then
		set YTDL_videoPW to script "Utilities2"'s get_Video_password(theButtonReturnLabel, theButtonOKLabel, MacYTDL_custom_icon_file_posix, diag_Title, MacYTDL_custom_icon_file)
		if YTDL_videoPW is "Main" then return
	end if
	-- v1.31, 23/4/26 - Moved netrc block to before Credentials so that YTDL_Use_netrc is correct
	if DL_Use_netrc is true then
		set YTDL_Use_netrc to "--netrc "
	else
		set YTDL_Use_netrc to ""
	end if
	-- v1.31, 23/4/26 - Added netrc-location
	if DL_Use_Netrc_Location is true then
		set YTDL_Use_netrc_Location to "--netrc-location " & DL_Netrc_Location & " "
	else
		set YTDL_Use_netrc_Location to ""
	end if
	set YTDL_credentials to ""
	-- v1.31, 16/4/26 - Added new runtime setting for passwords that are video specific
	if credentials_choice is true then
		-- v1.29.3, 13/6/25 - Added code needed to return to Main dialog
		-- v1.31, 23/4/26 - Add test to ensure credentials are not requested at same time as "--netrc"
		if DL_Use_netrc is true then
			set theKeepNetrcLabel to localized string "Keep netrc" from table "MacYTDL"
			set theKeepCredentialsLabel to localized string "Keep Credentials" from table "MacYTDL"
			set keep_netrc_question to button returned of (display dialog (localized string "You have “netrc” turned on but that is mutually exclusive with Credentials. Do you wish to keep Credentials or keep “netrc” ?") with title diag_Title buttons {theKeepCredentialsLabel, theKeepNetrcLabel} default button 2 with icon file MacYTDL_custom_icon_file giving up after 600)
			if keep_netrc_question is theKeepCredentialsLabel then
				set YTDL_Use_netrc to ""
				...
				set credentials_choice to false
			end if
		else
			set YTDL_credentials to run_Utilities_handlers's get_YTDL_credentials(theButtonReturnLabel, theButtonOKLabel, MacYTDL_custom_icon_file_posix, diag_Title, MacYTDL_custom_icon_file)
		end if
		if YTDL_credentials is "Main" then return
		-- v1.31, 23/4/26 - There has been a bug in here for a long while. Need to continue to download without credentials if User clicked on "Skip" in Credentials dialog
		-- Processing just continues on
		if YTDL_credentials is "Download" then set YTDL_credentials to ""
	end if
	...
	if DL_Get_Cookies is true then
		set YTDL_Get_Cookies to ("--cookies-from-browser " & DL_Cookies_Source & " ")
	else
		set YTDL_Get_Cookies to ""
	end if
	...
	if button_number_returned is 6 then -- Continue to download	or batch functions	
		if openBatch_chosen is true then
			set branch_execution to run_Batch_Handlers's open_batch_processing(folder_chosen, DL_Remux_format, subtitles_choice, YTDL_videoPW, YTDL_credentials, YTDL_subtitles, YTDL_STEmbed, YTDL_format, YTDL_recode_remux, YTDL_Merge_Format, YTDL_Remux_original, YTDL_description, YTDL_audio_only, YTDL_audio_codec, YTDL_over_writes, YTDL_Thumbnail_Write, YTDL_Thumbnail_Embed, YTDL_metadata, YTDL_limit_rate_value, YTDL_verbose, YTDL_TimeStamps, YTDL_Use_Proxy, YTDL_Get_Cookies, YTDL_Use_Cookies, YTDL_Custom_Settings, YTDL_Custom_Template, YTDL_no_part, YTDL_QT_Compat, DL_Use_YTDLP, YTDL_Resolution_Limit, YTDL_Use_Parts, YTDL_No_Warnings, ADL_Clear_Batch, batch_file, theButtonOKLabel, theButtonReturnLabel, theButtonDownloadLabel, MacYTDL_custom_icon_file_posix, diag_Title, window_Position, MacYTDL_custom_icon_file, screen_width, screen_height, YTDL_Use_netrc, YTDL_Use_netrc_Location, deno_version, YTDL_version)
			...
			download_video(shellPath, path_to_MacYTDL, MacYTDL_custom_icon_file, MacYTDL_custom_icon_file_posix, screen_width, screen_height, YTDL_simulate_file, URL_user_entered, URL_user_entered_from_auto_download, folder_chosen, diag_Title, DL_batch_status, DL_Remux_format, subtitles_choice, YTDL_videoPW, YTDL_credentials, YTDL_subtitles, YTDL_STEmbed, YTDL_format_pref, YTDL_format, YTDL_recode_remux, YTDL_Merge_Format, YTDL_Remux_original, YTDL_description, YTDL_audio_only, YTDL_audio_codec, YTDL_over_writes, YTDL_Thumbnail_Write, YTDL_Thumbnail_Embed, YTDL_metadata, YTDL_limit_rate_value, YTDL_verbose, YTDL_TimeStamps, YTDL_Use_Proxy, YTDL_Get_Cookies, YTDL_Use_Cookies, YTDL_Custom_Settings, YTDL_Custom_Template, YTDL_no_part, skip_Main_dialog, theButtonOKLabel, theButtonCancelLabel, theButtonDownloadLabel, theButtonReturnLabel, theButtonQuitLabel, theButtonContinueLabel, YTDL_QT_Compat, DL_Use_YTDLP, theBestLabel, YTDL_Resolution_Limit, YTDL_Use_Parts, YTDL_No_Warnings, ADL_Clear_Batch, user_on_old_os, X_position, deno_version, YTDL_Use_netrc, YTDL_Use_netrc_Location, YTDL_version)
...
on download_video(shellPath, path_to_MacYTDL, MacYTDL_custom_icon_file, MacYTDL_custom_icon_file_posix, screen_width, screen_height, YTDL_simulate_file, URL_user_entered, URL_user_entered_from_auto_download, folder_chosen, diag_Title, DL_batch_status, DL_Remux_format, subtitles_choice, YTDL_videoPW, YTDL_credentials, YTDL_subtitles, YTDL_STEmbed, YTDL_format_pref, YTDL_format, YTDL_recode_remux, YTDL_Merge_Format, YTDL_Remux_original, YTDL_description, YTDL_audio_only, YTDL_audio_codec, YTDL_over_writes, YTDL_Thumbnail_Write, YTDL_Thumbnail_Embed, YTDL_metadata, YTDL_limit_rate_value, YTDL_verbose, YTDL_TimeStamps, YTDL_Use_Proxy, YTDL_Get_Cookies, YTDL_Use_Cookies, YTDL_Custom_Settings, YTDL_Custom_Template, YTDL_no_part, skip_Main_dialog, theButtonOKLabel, theButtonCancelLabel, theButtonDownloadLabel, theButtonReturnLabel, theButtonQuitLabel, theButtonContinueLabel, YTDL_QT_Compat, DL_Use_YTDLP, theBestLabel, YTDL_Resolution_Limit, YTDL_Use_Parts, YTDL_No_Warnings, ADL_Clear_Batch, user_on_old_os, X_position, deno_version, YTDL_Use_netrc, YTDL_Use_netrc_Location, YTDL_version)
	...
	-- v1.31, 18/5/26 - Needed to get content of URL after replacing characters - single quotes were converted to reverse slashes
	--	display dialog "URL_user_entered: " & URL_user_entered
	
	
	-- v1.31, 12/4/26 - Remove quotes and smart quotes from URL - is a common mistake that shouldn't stop downloading
	if URL_user_entered contains "\"" or URL_user_entered contains "'" or URL_user_entered contains "“" or URL_user_entered contains "‘" or URL_user_entered contains "”" or URL_user_entered contains "’" then
	...
	if (URL_user_entered_clean contains "youtube" or URL_user_entered_clean contains "youtu.be") and deno_version is "Not installed" then
		if deno_exists is true then
			-- Get version of installed Deno and update plist
	...
	-- v1.30, 17/11/25 - Convert commas, tabs, returns and line breaks in the URL entered by user into spaces => enables multiple and parallel downloads with existing code
	...
	-- v1.31, 6/4/26 - Add --flat-playlist to simulate which is much faster - can't return file extension which is really not needed - don't need alert
	-- v1.31, 6/4/26 - Add a too many playlist items check for parallel downloads - can't remember why it was omitted.
	...
	-- Does the URL point to a single playlist item ? If so, add --no-playlist which will be passed to yt-dlp; otherwise find out playlist name and how many items
	if URL_user_entered_clean contains "&index=" and URL_user_entered_clean contains "youtu" then
		...
		-- Does the URL point to a playlist ? If so, get name of playlist (which is also used as a flag) and number of items
		...
		try
			set playlist_Simulate to do shell script shellPath & "cd " & quoted form of downloadsFolder_Path & " ; " & DL_Use_YTDLP & " --flat-playlist " & DL_Playlist_Items_Spec & YTDL_Use_netrc & YTDL_Use_netrc_Location & YTDL_Use_Cookies & YTDL_No_Warnings & URL_user_entered_clean_quoted
			
			-- v1.31, 5/4/26 - Commented this try block previous do shell call finished much quicker than I thought - any way, not needed with fast playlist simulates
			--	try
		...
		-- v1.31, Apply soft limit on number of playlist items
		if playlist_Number_Items is greater than DL_PL_Items_Limit then
			--	try
	...
	-- v1.31, 6/4/26 - Add --flat-playlist to simulate - much faster but can't supply file extension which is really not needed - omit the alert too as simulate is too quick
	-- v1.31, 9/4/26 - Can't remember why playlist_Name is being set to "Parallel playlist". Comment out until it causes an issue.
	-- v1.31, Need to limit this block to playlists ! Using playlist_Name as flag
	--	else if DL_Parallel is true then -- can't remember what this was for !
	if DL_Parallel is true and playlist_Name is not "" then
		...
		--	set alerterPiD to do shell script quoted form of (resourcesPath & "alerter") & " -message " & playListAlertMessage & " -title " & playListAlertTitle & " -subtitle " & playListAlertSubtitle & " -sender com.apple.script.id.MacYTDL -actions " & playListAlertActionLabel & " -timeout 20 > /dev/null 2> /dev/null & echo $!"
		try
			--	do shell script shellPath & "cd " & quoted form of downloadsFolder_Path & " ; export LC_CTYPE=UTF-8 ; " & DL_Use_YTDLP & " --print '%(playlist_title)s##%(webpage_url)s##%(title)s.%(ext)s' " & DL_Playlist_Items_Spec & YTDL_Use_netrc & YTDL_Use_netrc_Location & YTDL_No_Warnings & YTDL_Use_Cookies & URL_user_entered_clean_quoted & " 2>&1 &>" & quoted form of YTDL_simulate_file & " ; exit 0"
			do shell script shellPath & "cd " & quoted form of downloadsFolder_Path & " ; export LC_CTYPE=UTF-8 ; " & DL_Use_YTDLP & " --flat-playlist --print '%(playlist_title)s##%(webpage_url)s##%(title)s' " & DL_Playlist_Items_Spec & YTDL_Use_netrc & YTDL_Use_netrc_Location & YTDL_No_Warnings & YTDL_Use_Cookies & URL_user_entered_clean_quoted & " 2>&1 &>" & quoted form of YTDL_simulate_file & " ; exit 0"
			--	try
			--		do shell script "kill " & alerterPiD
			--	end try
		on error error_Message
	...
	-- Separate simulate for playlists - enables much faster process
	-- Exclude parallel playlists - simulate file already contains required data
	-- Add is_live to output template to get live status into file name - that is stripped from output template later
	set simulate_YTDL_output_template to run_Utilities_handlers's replace_chars(YTDL_output_template, " -o '%", " -o '%(is_live)s#%")
	if parallel_playlist is false and playlist_Name is not "" then
		do shell script shellPath & "cd " & quoted form of downloadsFolder_Path & " ; export LC_CTYPE=UTF-8 ; " & DL_Use_YTDLP & " --flat-playlist --print '%(playlist_title)s##%(webpage_url)s##%(title)s' " & YTDL_format_pref & DL_Playlist_Items_Spec & YTDL_videoPW & YTDL_credentials & YTDL_Use_netrc & YTDL_Use_netrc_Location & YTDL_Use_Proxy & YTDL_Use_Cookies & YTDL_no_playlist & YTDL_No_Warnings & simulate_YTDL_output_template & " " & URL_user_entered_clean_quoted & " 2>&1 &>" & quoted form of YTDL_simulate_file & " ; exit 0"
	end if
	-- Two mutually exclusive simulate calls
	if parallel_playlist is false and playlist_Name is "" then
		do shell script shellPath & "cd " & quoted form of downloadsFolder_Path & " ; export LC_CTYPE=UTF-8 ; " & DL_Use_YTDLP & " --get-filename --ignore-errors " & YTDL_format_pref & DL_Playlist_Items_Spec & YTDL_videoPW & YTDL_credentials & YTDL_Use_netrc & YTDL_Use_netrc_Location & YTDL_Use_Proxy & YTDL_Use_Cookies & YTDL_no_playlist & YTDL_No_Warnings & simulate_YTDL_output_template & " " & URL_user_entered_clean_quoted & " 2>&1 &>" & quoted form of YTDL_simulate_file & " ; exit 0"
	...
	if (length of YTDL_simulate_log is greater than 195) and ((count of paragraphs in YTDL_simulate_log) is less than 3) then
		...
		do shell script shellPath & "cd " & quoted form of downloadsFolder_Path & " ; export LC_CTYPE=UTF-8 ; " & DL_Use_YTDLP & " --get-filename --ignore-errors " & YTDL_no_playlist & DL_Playlist_Items_Spec & YTDL_format_pref & YTDL_videoPW & YTDL_credentials & YTDL_Use_netrc & YTDL_Use_netrc_Location & YTDL_Use_Proxy & YTDL_Use_Cookies & YTDL_No_Warnings & simulate_YTDL_output_template & " " & URL_user_entered_clean_quoted & " 2>&1 &>" & quoted form of YTDL_simulate_file & " ; exit 0"
		-- Added delay as one user gets end of file errors which might be due to simulate file not being ready
		delay 1
		try
			set YTDL_simulate_log to read POSIX file YTDL_simulate_file as «class utf8»
	...
	if YTDL_simulate_log contains "Unsupported URL: https://10.com.au" then
	...
	if YTDL_simulate_log contains "requested format not available" then
		...
		if quit_or_return is theButtonReturnLabel then
			...
			do shell script shellPath & "cd " & quoted form of downloadsFolder_Path & " ; export LC_CTYPE=UTF-8 ; " & DL_Use_YTDLP & " --get-filename " & YTDL_no_playlist & DL_Playlist_Items_Spec & YTDL_videoPW & YTDL_credentials & YTDL_Use_netrc & YTDL_Use_netrc_Location & YTDL_No_Warnings & URL_user_entered_clean_quoted & " " & simulate_YTDL_output_template & " > /dev/null" & " &> " & quoted form of YTDL_simulate_file
			set YTDL_format to ""
			-- Added delay as one user gets end of file errors which might be due to simulate file not being ready
			delay 1
			try
	...
	-- v1.31, 14/4/26 - Simplify calls to replace_chars which I forgot can take a list
	set YTDL_simulate_log to run_Utilities_handlers's replace_chars(YTDL_simulate_log, {"True#", "False#", "NA#"}, "")
	...
	if YTDL_simulate_log contains "ERROR:" and YTDL_simulate_log does not contain "Unsupported URL: https://www.sbs.com.au/ondemand" and YTDL_simulate_log does not contain "Unsupported URL: https://iview.abc.net.au/show" then
		...
		if URL_user_entered_clean contains "https://iview.abc.net.au/show" and YTDL_simulate_log contains "An extractor error has occurred" then
			...
			if skip_Main_dialog is true then
				...
				-- v1.29.1 - If error caused by problem with cookies, only choices are to return to Main or Quit
				if YTDL_simulate_log contains "Use --cookies-from-browser" then
					set theURLErrorTextLabel3 to localized string "Quit or OK to return to Main Dialog." from table "MacYTDL"
	...
	if DL_formats_list is true and DL_batch_status is false then
		...
		-- v1.30, 5/11/25- Commented out as Formats library now 'used' instead of loaded
		...
		set chosen_formats_list to run_Formats_Chooser_Handler's formats_Chooser(URL_user_entered, diag_Title, theButtonCancelLabel, theButtonDownloadLabel, X_position, screen_height, MacYTDL_custom_icon_file, MacYTDL_custom_icon_file_posix, theButtonReturnLabel, skip_Main_dialog, path_to_MacYTDL, DL_Use_YTDLP, shellPath, download_filename_formats, YTDL_videoPW, YTDL_credentials, window_Position, formats_reported, is_Livestream_Flag, YTDL_Use_netrc, YTDL_Use_netrc_Location)
	...
	set ytdl_settings to quoted form of (" --ignore-errors --newline " & YTDL_subtitles & YTDL_STEmbed & YTDL_videoPW & YTDL_credentials & YTDL_format & YTDL_Resolution_Limit & YTDL_Use_Parts & YTDL_No_Warnings & YTDL_recode_remux & YTDL_Merge_Format & YTDL_Remux_original & YTDL_description & YTDL_audio_only & YTDL_audio_codec & YTDL_over_writes & YTDL_Thumbnail_Write & YTDL_Thumbnail_Embed & YTDL_metadata & YTDL_limit_rate_value & YTDL_verbose & YTDL_Use_Proxy & YTDL_Get_Cookies & YTDL_Use_Cookies & YTDL_no_playlist & YTDL_no_part & YTDL_Custom_Settings & YTDL_output_template & " " & YTDL_QT_Compat & " " & YTDL_formats_to_download & " " & YTDL_Use_netrc & YTDL_Use_netrc_Location & " ")
				...
				if length_each_filename is not 0 then
					...
					if search_for_download is not {} then
						...
						if overwrite_continue_choice is theABCShowExistsButtonOverwriteLabel then
							...
							set ytdl_settings to quoted form of (" --ignore-errors --newline " & YTDL_subtitles & YTDL_STEmbed & YTDL_videoPW & YTDL_credentials & YTDL_format & YTDL_Resolution_Limit & YTDL_Use_Parts & YTDL_No_Warnings & YTDL_recode_remux & YTDL_Merge_Format & YTDL_Remux_original & YTDL_description & YTDL_audio_only & YTDL_audio_codec & YTDL_metadata & YTDL_limit_rate_value & YTDL_verbose & YTDL_Use_Proxy & YTDL_Get_Cookies & YTDL_Use_Cookies & YTDL_no_playlist & YTDL_no_part & YTDL_Custom_Settings & YTDL_output_template & " " & YTDL_QT_Compat & " " & YTDL_formats_to_download & " " & YTDL_Use_netrc & YTDL_Use_netrc_Location & " ")
							...
							set ytdl_settings to quoted form of (" --ignore-errors --newline " & YTDL_subtitles & YTDL_STEmbed & YTDL_videoPW & YTDL_credentials & YTDL_format & YTDL_Resolution_Limit & YTDL_Use_Parts & YTDL_No_Warnings & YTDL_recode_remux & YTDL_Merge_Format & YTDL_Remux_original & YTDL_description & YTDL_audio_only & YTDL_audio_codec & YTDL_over_writes & YTDL_metadata & YTDL_limit_rate_value & YTDL_verbose & YTDL_Use_Proxy & YTDL_Get_Cookies & YTDL_Use_Cookies & YTDL_no_playlist & YTDL_no_part & YTDL_Custom_Settings & YTDL_output_template_new & " " & YTDL_QT_Compat & " " & YTDL_formats_to_download & " " & YTDL_Use_netrc & YTDL_Use_netrc_Location & " ")
	...
	-- v1.31, 18/5/26 - This used for debugging error in creating log file for parallel playlist item	
	--	display dialog "download_filename_new: " & download_filename_new
...
on set_File_Names(shellPath, YTDL_simulate_log, URL_user_entered, ABC_show_name, SBS_show_name, DL_Use_YTDLP, parallel_playlist, number_of_URLs)
	...
	-- First, look for non-iView and OnDemand show pages (but iView non-error single downloads are included)
	if ABC_show_name is "" and SBS_show_name is "" then -- not an ABC or SBS show page
		if number_of_URLs is 1 and parallel_playlist is false then -- Single file download or playlist to be downloaded serially
			...
			if YTDL_simulate_log does not contain "WARNING:" and YTDL_simulate_log does not contain "[Warning] " and YTDL_simulate_log does not contain "ERROR:" then --<= A single file or playlist download non-error and non-warning (iView and non-iView)
			...
			if DL_Parallel is true and parallel_playlist is false then
				...
				repeat with x from 1 to number_paragraphs
					if paragraph x of YTDL_simulate_log does not contain "WARNING:" and paragraph x of YTDL_simulate_log does not contain "[Private video]" then
						...
						-- v1.31, 14/4/26 - Simplify calls to replace_char
						...
						-- v1.31, 18/5/26 - Decided to remove slashes from file names - Looked ugly and POSIX conversions turned them into colons which cause errors later on
		...
		-- v1.31, 9/4/26 - Need to stop single non-playlist downloads coming into here
		if parallel_playlist is true and playlist_Name is not "" then
			
			
			--	display dialog "parallel_playlist is true and playlist_Name is: " & playlist_Name
			...
			repeat with x from 1 to number_paragraphs
				if paragraph x of YTDL_simulate_log does not contain "WARNING:" and paragraph x of YTDL_simulate_log does not contain "[Private video]" then
					...
					-- Add a unique delimiter to signify this is a playlist parallel download - used by Monitor - double hash delimiter might be a problem as it's also used by yt-dlp template's print
...
-- v1.31, 15/2/25 - Added Get cookies, repositioned auto_subtitles and moved bottom rule further down to make more visible
on set_settings()
	...
	--	set {settings_theCheckbox_Use_CustomSettings, theTop, BoxCustSetLeftDist} to create checkbox theCheckBoxUseCustomSettingsLabel left inset 70 bottom (theTop + 7) max width 150 initial state DL_Use_Custom_Settings
	--	set theFieldCustomSettingsLabel to localized string "Custom settings" from table "MacYTDL"
	--	set {settings_theField_Custom_Settings, theTop} to create field DL_Custom_Settings left inset (BoxCustSetLeftDist + 80) bottom (theTop - 20) field width 180 placeholder text theFieldCustomSettingsLabel
	
	set theCheckBoxUseCustomTemplateLabel to localized string "Use custom template" from table "MacYTDL"
	set {settings_theCheckbox_Use_CustomTemplate, theTop, CustTempPopLeftDist} to create checkbox theCheckBoxUseCustomTemplateLabel left inset 70 bottom (theTop + 7) max width 150 initial state DL_Use_Custom_Template
	set theFieldCustomTemplateLabel to localized string "Custom file name template" from table "MacYTDL"
	set {settings_theField_Custom_Template, theTop} to create field DL_Custom_Template left inset (CustTempPopLeftDist + 80) bottom (theTop - 20) field width 180 placeholder text theFieldCustomTemplateLabel
	
	set theCheckBoxGetCookiesLabel to localized string "Get cookies" from table "MacYTDL"
	set {settings_theCheckbox_Get_Cookies, theTop} to create checkbox theCheckBoxGetCookiesLabel left inset 70 bottom (theTop + 3) max width 150 initial state DL_Get_Cookies
	--	set theFieldCookiesSourceLabel to localized string "Browser name" from table "MacYTDL"
	--	set {settings_theField_Cookies_Source, theTop} to create field DL_Cookies_Source left inset 185 bottom (theTop - 20) field width 150 placeholder text theFieldCookiesSourceLabel
	set thePopupGetCookiesLabel to localized string "Browser:" from table "MacYTDL"
	set {settings_thePopup_Get_Cookies, settings_get_cookies_label, theTop} to create labeled popup {"Safari", "Firefox", "Chrome", "Edge", "Chromium", "Opera", "Vivaldi", "Brave", "Whale"} left inset 180 bottom (theTop - 23) popup width 100 max width 200 label text thePopupGetCookiesLabel popup left 70 initial choice DL_Cookies_Source
	...
	set {settings_theField_ProxyURL, theTop} to create field DL_Proxy_URL left inset (ProxyCheckBoxWidth + 85) bottom (theTop - 20) field width 250 placeholder text theFieldProxyURLPlaceholderLabel
	...
	-- v1.31, 1/5/26 - Add merge output format; Add a rule below this group
	set {theSettingsConvertRule, theTop} to create rule (theTop + 2) left inset 70 rule width (accViewWidth - 70)
	set theLabeledPopupRemuxFormatLabel to localized string "Format:" from table "MacYTDL"
	set theCheckboxSetMergeFormatLabel to localized string "Set merge format" from table "MacYTDL"
	set {settings_theCheckbox_Merge, theTop} to create checkbox theCheckboxSetMergeFormatLabel left inset 70 bottom (theTop + 5) max width 200 initial state DL_Set_Merge_Format
	set {settings_thePopup_Merge_Format, settings_merge_formats_label, theTop} to create labeled popup {theDefaultLabel, "avi", "flv", "mkv", "mov", "mp4", "webm"} bottom (theTop - 23) initial choice DL_Merge_Format label text theLabeledPopupRemuxFormatLabel left inset 250 max width 200 popup left 70 popup width 100
	if DL_Use_YTDLP is "yt-dlp" then
	...
	set {settings_theCheckbox_AutoSubTitles, theTop} to create checkbox theCheckboxDLAutoSTsLabel left inset 235 bottom (theTop + 3) max width 250 initial state DL_YTAutoST
	set theCheckboxEmbedSTsLabel to localized string "Embed subtitles" from table "MacYTDL"
	set {settings_theCheckbox_STEmbed, theTop} to create checkbox theCheckboxEmbedSTsLabel left inset 70 bottom (theTop - 18) max width 250 initial state DL_STEmbed
	...
	set {settings_thePopUp_FileFormat, settings_formatlabel, theTop, popupLeft} to create labeled popup {theDefaultLabel, "mp4", "webm", "ogg", "3gp", "flv", "aac", "m4a", "mp3", "wav"} left inset 70 bottom (theTop + 3) popup width 90 max width 400 label text theLabeledPopUpFileFormatLabel popup left 70 initial choice DL_format
	...
	-- allControls before removing custom settings controls
	--	set settings_allControls to {theSettingsRule, settings_theCheckbox_Use_CustomSettings, settings_theField_Custom_Settings, settings_theCheckbox_Use_CustomTemplate, settings_theField_Custom_Template, settings_theCheckbox_Get_Cookies, settings_thePopup_Get_Cookies, settings_get_cookies_label, settings_theCheckBox_Use_Cookies, settings_theCookiesLocationPathControl, settings_theCheckBox_Use_Proxy, settings_theField_ProxyURL, theSettingsConvertRule, settings_theCheckbox_Original, settings_theCheckbox_Merge, settings_thePopup_Merge_Format, settings_merge_formats_label, theMatrixLabel, settings_theMatrix_Recode_Remux, settings_thePopUp_RemuxFormat, settings_remuxlabel, settings_theCheckbox_Formats, settings_theCheckbox_Metadata, settings_theCheckbox_ThumbEmbed, settings_theCheckbox_ThumbWrite, settings_theCheckbox_AutoSubTitles, settings_thePopUp_SubTitlesFormat, settings_STFormatlabel, settings_theField_STLanguage, settings_language_label, settings_theCheckbox_STEmbed, settings_theCheckbox_SubTitles, settings_theCheckbox_AudioOnly, settings_theCheckbox_OverWrites, settings_thePopup_AudioCodec, settingsCodecLabel, settings_theCheckbox_Description, theSettingsAdminRule, settings_thePopUp_Resolution_Limit, settings_resolutionlabel, settings_thePopUp_FileFormat, settings_formatlabel, settings_thePathControl, settings_pathLabel, MacYTDL_icon, settings_prompt, theSettingsSTEndRule}
	
	set settings_allControls to {theSettingsRule, settings_theCheckbox_Use_CustomTemplate, settings_theField_Custom_Template, settings_theCheckbox_Get_Cookies, settings_thePopup_Get_Cookies, settings_get_cookies_label, settings_theCheckBox_Use_Cookies, settings_theCookiesLocationPathControl, settings_theCheckBox_Use_Proxy, settings_theField_ProxyURL, theSettingsConvertRule, settings_theCheckbox_Original, settings_theCheckbox_Merge, settings_thePopup_Merge_Format, settings_merge_formats_label, theMatrixLabel, settings_theMatrix_Recode_Remux, settings_thePopUp_RemuxFormat, settings_remuxlabel, settings_theCheckbox_Formats, settings_theCheckbox_Metadata, settings_theCheckbox_ThumbEmbed, settings_theCheckbox_ThumbWrite, settings_theCheckbox_AutoSubTitles, settings_thePopUp_SubTitlesFormat, settings_STFormatlabel, settings_theField_STLanguage, settings_language_label, settings_theCheckbox_STEmbed, settings_theCheckbox_SubTitles, settings_theCheckbox_AudioOnly, settings_theCheckbox_OverWrites, settings_thePopup_AudioCodec, settingsCodecLabel, settings_theCheckbox_Description, theSettingsAdminRule, settings_thePopUp_Resolution_Limit, settings_resolutionlabel, settings_thePopUp_FileFormat, settings_formatlabel, settings_thePathControl, settings_pathLabel, MacYTDL_icon, settings_prompt, theSettingsSTEndRule}
	...
	if (CustTempPopLeftDist + 250) is greater than calculatedAccViewWidth then
		set calculatedAccViewWidth to (CustTempPopLeftDist + 250)
	end if
	tell me to activate
	...
	-- v1.31, 2/5/26 - Finished adding cookies source and moving custom settings to Admin dialog
	...
	if settings_button_number_returned is 3 or settings_button_number_returned is 2 then -- Save Settings and return to Main or Admin
		...
		-- set settings_choice_1 to item 1 of settings_controls_results -- <= The settings ruled line
		set settings_use_custom_template_choice to item 2 of settings_controls_results -- <= Use custom template choice
		set settings_custom_template_choice to item 3 of settings_controls_results -- <= The custom template to use
		set settings_get_cookies_choice to item 4 of settings_controls_results -- <= Get cookies choice
		set settings_cookies_source_choice to item 5 of settings_controls_results -- <= Browser for cookies location
		-- set settings_choice_6 to item 6 of settings_controls_results -- <= The cookies source popup label
		set settings_use_cookies_choice to item 7 of settings_controls_results -- <= Use cookies file choice
		set settings_cookies_location_choice to item 8 of settings_controls_results -- <= Cookies file location
		set settings_use_proxy_choice to item 9 of settings_controls_results -- <= Use proxy choice
		set settings_proxy_URL_choice to item 10 of settings_controls_results -- <= The proxy URL
		-- set settings_choice_11 to item 11 of settings_controls_results -- <= The convert ruled line
		set settings_original_choice to item 12 of settings_controls_results -- <= Keep original after remux choice
		set settings_set_merge_format_choice to item 13 of settings_controls_results -- <= Set the merge output format choice
		set settings_merge_format_choice to item 14 of settings_controls_results -- <= The merge output format choice
		-- set settings_choice_15 to item 15 of settings_controls_results -- <= The merge output format label
		-- set settings_choice_16 to item 16 of settings_controls_results -- <= Remux/Recode matrix label
		set settings_remux_recode_choice to item 17 of settings_controls_results -- <= Remux/Recode choice
		set settings_remux_format_choice to item 18 of settings_controls_results -- <= Remux/Recode format choice
		-- set settings_choice_19 to item 19 of settings_controls_results -- <= The Remux format popup label
		set settings_formats_list_choice to item 20 of settings_controls_results -- <= Get formats list choice
		set settings_metadata_choice to item 21 of settings_controls_results -- <= Add metadata choice
		...
		-- set settings_STFormatlabel_choice to item 26 of settings_controls_results -- <= Subtitles format popup label
		set settings_subtitleslanguage_choice to item 27 of settings_controls_results -- <= Subtitles language choice
		-- set settings_subtitleslanguage_28 to item 28 of settings_controls_results -- <= Subtitles language field label
		...
		-- set settings_choice_36 to item 36 of settings_controls_results -- <= The Admin rule
		set settings_resolution_choice to item 37 of settings_controls_results -- <= Maximum resolution choice
		-- set settings_choice_38 to item 38 of settings_controls_results -- <= The Format popup label
		set settings_format_choice to item 39 of settings_controls_results -- <= File format choice
		-- set settings_choice_40 to item 40 of settings_controls_results -- <= The Format popup label
		set settings_folder_choice to item 41 of settings_controls_results -- <= The download path choice
		-- set settings_choice_42 to item 42 of settings_controls_results -- <= The Path label
		-- set settings_choice_43 to item 43 of settings_controls_results -- <= The MacYTDL icon
		-- set settings_choice_44 to item 44 of settings_controls_results -- <= The Settings label
		-- set settings_choice_45 to item 45 of settings_controls_results -- <= The Subtitles end rule
		
		-- Test new settings work
		--	display dialog "settings_set_merge_format_choice: " & settings_set_merge_format_choice & return & "settings_merge_format_choice: " & settings_merge_format_choice & return & "settings_folder_choice: " & settings_folder_choice
		...
		tell application "System Events"
			tell property list file MacYTDL_prefs_file
				...
				set value of property list item "Cookies_Source" to settings_cookies_source_choice
				...
				set value of property list item "Get_Cookies" to settings_get_cookies_choice
				set value of property list item "Cookies_Source" to settings_cookies_source_choice
		...
		-- v1.31, 1/5/26 - Check that format is set if user wants to set the merge output format
		if settings_set_merge_format_choice is true and settings_merge_format_choice is theDefaultLabel then
			set theNeedMergeFormatLabel to localized string "Sorry, you need to choose a format for merging output." from table "MacYTDL"
			display dialog theNeedMergeFormatLabel with title diag_Title buttons {theButtonOKLabel} default button 1 with icon file MacYTDL_custom_icon_file giving up after 600
			set_settings()
		else
			tell application "System Events"
				tell property list file MacYTDL_prefs_file
					set value of property list item "Set_Merge_Format" to settings_set_merge_format_choice
					set value of property list item "Merge_Format" to settings_merge_format_choice
				end tell
			end tell
		end if
		
		-- User wants a formats list - if user switches off formats list toggle the saved setting
		-- v1.26.1 – For some reason, formats list and auto download now crash on Sonoma as well as Monterey, but not Ventura - decided not to block for now
		...
		-- v1.31, 29/4/26 - Implemented logic redesign to ensure settings are saved if user corrects mistake
		...
		if settings_use_proxy_choice is true then
			if settings_proxy_URL_choice is "" then
				...
				set settings_use_proxy_choice to false
				set_settings()
			end if
			...
			if settings_use_proxy_choice is true then
				tell application "System Events"
					tell property list file MacYTDL_prefs_file
						set value of property list item "Proxy_URL" to settings_proxy_URL_choice
						set value of property list item "Use_Proxy" to true
					end tell
				end tell
			end if
		else
			tell application "System Events"
				tell property list file MacYTDL_prefs_file
					--		set value of property list item "Proxy_URL" to settings_proxy_URL_choice
					set value of property list item "Use_Proxy" to false
				end tell
			end tell
		end if
		
		-- Check user has supplied name of browser from which to source cookies
		-- v1.31, 29/4/26 - Implemented logic redesign to ensure settings are saved if user corrects mistake ====>>>> 2/5/26 - No need for error checking with popup which defaults to Safari
		--		if settings_get_cookies_choice is true then
		--			if settings_cookies_source_choice does not contain "brave" and settings_cookies_source_choice does not contain "chrome" and settings_cookies_source_choice does not contain "chromium" and settings_cookies_source_choice does not contain "edge" and settings_cookies_source_choice does not contain "firefox" and settings_cookies_source_choice does not contain "opera" and settings_cookies_source_choice does not contain "safari" and settings_cookies_source_choice does not contain "vivaldi" and settings_cookies_source_choice does not contain "whale" then
		--				set theMustProvideCookiesSourceLabel to localized string "Sorry, MacYTDL can only get cookies from one of these web browsers:" & return & return & "brave, chrome, chromium, edge, firefox,  opera, safari, vivaldi and whale" from table "MacYTDL"
		--				display dialog theMustProvideCookiesSourceLabel with title diag_Title buttons {theButtonOKLabel} default button 1 with icon file MacYTDL_custom_icon_file giving up after 600
		--				set settings_get_cookies_choice to false
		--				set_settings()
		--			end if
		--			-- Now can go ahead and set the cookies source settings
		--			if settings_get_cookies_choice is true then
		--				tell application "System Events"
		--					tell property list file MacYTDL_prefs_file
		--						set value of property list item "Get_Cookies" to true
		--						set value of property list item "Cookies_Source" to settings_cookies_source_choice
		--					end tell
		--				end tell
		--			end if
		...
		--			tell application "System Events"
		--				tell property list file MacYTDL_prefs_file
		--					set value of property list item "Get_Cookies" to false
		--					--		set value of property list item "Cookies_Source" to ""
		--				end tell
		--			end tell
		--		end if
		...
		if settings_use_cookies_choice is true then
			if settings_cookies_location_choice is ("/" & theNoCookieFileLabel) then
				...
				set settings_use_cookies_choice to false
				set_settings()
			end if
			...
			if settings_use_cookies_choice is true then
				tell application "System Events"
					tell property list file MacYTDL_prefs_file
						set value of property list item "Use_Cookies" to true
						set value of property list item "Cookies_Location" to settings_cookies_location_choice
					end tell
				end tell
			end if
		else
			-- Ask if user wants to blank out the cookies location - there's no way for user to blank out the field - so most do it in code
			...
			tell application "System Events"
				tell property list file MacYTDL_prefs_file
					set value of property list item "Use_Cookies" to false
					set value of property list item "Cookies_Location" to settings_cookies_location_choice
				end tell
			end tell
		end if
		...
		-- v1.31, 29/4/26 - Implemented logic redesign to ensure settings are saved if user corrects mistake
		if settings_use_custom_template_choice is true then
			if (settings_custom_template_choice is "" or settings_custom_template_choice is " ") then
				...
				set settings_use_custom_template_choice to false
				set_settings()
			end if
			...
			if settings_use_custom_template_choice is true then
				...
				tell application "System Events"
					tell property list file MacYTDL_prefs_file
						set value of property list item "Use_Custom_Output_Template" to true
						set value of property list item "Custom_Output_Template" to settings_custom_template_choice
					end tell
				end tell
			end if
			...
			tell application "System Events"
				tell property list file MacYTDL_prefs_file
					set value of property list item "Use_Custom_Output_Template" to false
					--		set value of property list item "Custom_Output_Template" to settings_custom_template_choice
				end tell
			end tell
		end if
		...
		-- v1.31, 29/4/26 - Implemented logic redesign to ensure settings are saved if user corrects mistake
		-- v1.31, 2/5/26 - Moved custom settings to Admin dialog
		--		if settings_use_custom_settings_choice is true then
		--			if settings_custom_settings_choice is "" then
		...
		--				set settings_use_custom_settings_choice to false
		--				set_settings()
		--			end if
		...
		--				tell application "System Events"
		--					tell property list file MacYTDL_prefs_file
		--						set value of property list item "Use_Custom_Settings" to true
		--						set value of property list item "Custom_Settings" to settings_custom_settings_choice
		--					end tell
		--				end tell
		--			end if
		...
		--			tell application "System Events"
		--				tell property list file MacYTDL_prefs_file
		--					set value of property list item "Use_Custom_Settings" to false
		--					--	set value of property list item "Custom_Settings" to settings_custom_settings_choice
		--				end tell
		--			end tell
		--		end if
	...
	-- return  -- This would return processing to location from where it was called which is after the main dialog <= old code made obsolete by v1.29.2
...
-- Handler to check and update stable edition of yt-dlp if user wishes - called by Utilities dialog to update script and switch to yt-dlp, the auto check on startup and the Warning dialog - Not called if user has Homebrew
-- v1.24 - No longer offer to update youtube-dl
-- v1.24 – Added code to enable switch from Homebrew to MacYTDL yt-dlp install
-- v1.29 - Changed yt-dlp install for users on macOS 10.15+ to unpacked edition - much faster
...
-- v1.31, 12/4/26 - Fixed URL - was set to nightly - now points to stable
on check_ytdl(DL_Use_YTDLP)
	...
	-- v1.31, 6/4/26 - Extended if block to end of Try block so as to exclude Legacy users from this curl - don't want to make all users ignore certificates - otherwise set YTDL_releases_page to "Null"
	-- It's all rather tricky: show_yt_dlp contains either yt-dlp or yt-dlp-legacy; DL_Use_YTDLP contains eith yt-dlp or youtube-dl
	if show_yt_dlp is "yt-dlp" then
		...
		set YTDL_releases_page to "Null" -- For users on Legacy versions of yt-dlp
	end if
	...
	-- Need to check installed version for both current and legacy versions of yt-dlp as well as youtube-dl
	if DL_Use_YTDLP is "yt-dlp" or show_yt_dlp is "yt-dlp-legacy" then
		-- v1.31, 15/4/26 - Current date used to test timing - results not useful
		--	set version_start_time to current date
		set installedYTDL_version to do shell script ytdlp_file & " --version"
		--	set version_end_time to current date
		--	display dialog "version_start_time: " & version_start_time & return & "version_end_time: " & version_end_time
	...
	if YTDL_releases_page is "" then
		...
		-- v1.31, 6/4/26 - Only get current yt-dlp version for macOS 15+ - test installed Legacy version against 2025.08.11
		if show_yt_dlp is "yt-dlp" then
			set AppleScript's text item delimiters to "Latest"
			set YTDL_releases_text to text item 1 of YTDL_releases_page
			set numParas to count paragraphs in YTDL_releases_text
			...
			set YTDL_version_check to "2025.08.11"
		end if
		if YTDL_version_check is not equal to installedYTDL_version or homebrew_ytdlp_exists is true then
			...
			if YTDL_install_answ is theButtonYesLabel then
				try
					...
					-- v1.29 - Users on macOS 10.15+ get the current unpacked stable  version
					...
					if show_yt_dlp is "yt-dlp" then
						-- v1.31, 12/4/26 - pointing to wrong version of yt-dlp ! Nightly installs are updated in Utilities()
						--	do shell script "curl -L https://github.com/yt-dlp/yt-dlp-nightly-builds/releases/latest/download/yt-dlp_macos.zip -o /usr/local/bin/yt-dlp_macos.zip" with administrator privileges
...
on utilities()
	...
	if ytdlp_exists is true and DL_Use_YTDLP is "yt-dlp" and show_yt_dlp is not "yt-dlp-legacy" then
		...
		set theCheckInstallTDLLabel to localized string "Install" from table "MacYTDL"
		if ytdlp_kind is "stable" then
			set theCheckBoxCheckYTDLNightlyLabel to theCheckInstallTDLLabel & " " & (localized string "nightly build" from table "MacYTDL")
		...
		if ytdlp_kind is "nightly" then
			...
			set theCheckBoxCheckYTDLLabel to theCheckInstallTDLLabel & " " & (localized string "stable build" from table "MacYTDL")
	...
	else if ytdlp_exists is true and show_yt_dlp is "yt-dlp-legacy" and YTDL_version is not "2025.08.11" then
	...
	if utilities_button_number_returned is 5 then -- Start
		...
		if utilities_YTDL_webpage_choice is true then
			if DL_Use_YTDLP is "youtube-dl" then
				...
				open location "https://github.com/yt-dlp/yt-dlp/blob/master/README.md"
			end if
		end if
