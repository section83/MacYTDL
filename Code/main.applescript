...
-- Set variables to contain user's macOS version - for Utilities, Settings, FFmpeg install and yt-dlp install - check if user is on macOS 10.10 to 10.14.6 for YT-DLP legacy install + Utilities dialog - If user on <10.12.1 will have expired certificates preventing FFmpeg dowload - Users on 12.3+ cannot use youtube-dl
...
on main_dialog()
	...
	set {theButtons, minWidth} to create buttons {theButtonsHelpLabel, theButtonsUtilitiesLabel, theButtonQuitLabel, theButtonsAdminLabel, theButtonsSettingsLabel, theButtonContinueLabel} button keys {"?", "u", "q", "a", "s", ""} default button 6
...
on download_video(shellPath, path_to_MacYTDL, MacYTDL_custom_icon_file, MacYTDL_custom_icon_file_posix, screen_width, screen_height, YTDL_simulate_file, URL_user_entered, URL_user_entered_from_auto_download, folder_chosen, diag_Title, DL_batch_status, remux_format_choice, subtitles_choice, YTDL_credentials, YTDL_subtitles, YTDL_STEmbed, YTDL_format_pref, YTDL_format, YTDL_remux_format, YTDL_Remux_original, YTDL_description, YTDL_audio_only, YTDL_audio_codec, YTDL_over_writes, YTDL_Thumbnail_Write, YTDL_Thumbnail_Embed, YTDL_metadata, YTDL_limit_rate_value, YTDL_verbose, YTDL_TimeStamps, YTDL_Use_Proxy, YTDL_Use_Cookies, YTDL_Custom_Settings, YTDL_Custom_Template, YTDL_no_part, skip_Main_dialog, theButtonOKLabel, theButtonCancelLabel, theButtonDownloadLabel, theButtonReturnLabel, theButtonQuitLabel, theButtonContinueLabel, YTDL_QT_Compat, DL_Use_YTDLP, theBestLabel, YTDL_Resolution_Limit, YTDL_Use_Parts, YTDL_No_Warnings)
	...
	if URL_user_entered_clean contains "&index=" and URL_user_entered_clean contains "youtu" then
		...
		if DL_Parallel is false then
			...
			set alerterPiD to do shell script quoted form of (resourcesPath & "alerter") & " -message " & playListAlertMessage & " -title " & playListAlertTitle & " -subtitle " & playListAlertSubtitle & " -sender com.apple.script.id.MacYTDL -actions " & playListAlertActionLabel & " -timeout 20 > /dev/null 2> /dev/null & echo $!"
			try
				do shell script shellPath & "cd " & quoted form of downloadsFolder_Path & " ; export LC_CTYPE=UTF-8 ; " & DL_Use_YTDLP & " --print '%(playlist_title)s##%(webpage_url)s##%(title)s.%(ext)s' " & DL_Playlist_Items_Spec & YTDL_Use_Cookies & URL_user_entered_clean_quoted & " 2>&1 &>" & YTDL_simulate_file & " ; exit 0"
				try
					do shell script "kill " & alerterPiD
				end try
			on error error_Message
...
on set_settings()
	...
	set {theButtons, minWidth} to create buttons {theButtonCancelLabel, theSettingsBUttonAdminLabel, theButtonsSaveLabel} button keys {".", "a", ""} default button 3
	...
	if settings_button_number_returned is 3 or settings_button_number_returned is 2 then -- Save Settings and return to Main or Admin
		...
		-- User wants a formats list - check that Myriad Tables Lib is installed - if user switches off formats list toggle the saved setting - formats list and auto download seem to be compatible after all
...
on utilities()
	...
	if ytdlp_exists is true and (DL_Use_YTDLP is "yt-dlp" or DL_Use_YTDLP is "yt-dlp-legacy") then
		...
		set theCheckBoxDoNotUpdateYTDLLabel to localized string "Do not update YT-DLP" from table "MacYTDL"
		set theCheckBoxDoNotUpdateYTDLTextLabel to localized string "Update" from table "MacYTDL"
		set {utilities_theMatrix_YTDL_Check, theMatrixLabel, theTop, theMatrixWidth} to create labeled matrix {theCheckBoxDoNotUpdateYTDLLabel, theCheckBoxCheckYTDLversion, theCheckBoxCheckYTDLNightlyVersion} initial choice 1 bottom (theTop + 5) label text (theCheckBoxDoNotUpdateYTDLTextLabel & " YT-DLP") matrix left 110 max width 600
	else
